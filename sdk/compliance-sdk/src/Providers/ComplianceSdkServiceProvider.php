...
namespace Academorix\ComplianceSdk\Providers;
...
/**
 * Root provider for `academorix/compliance-sdk`.
 *
 * Builds the Compliance service's own Saloon connector from `sdk.compliance.*`,
 * runs a discovery pass scoped to `#[AsSdkResource(service: 'compliance')]`, and
 * binds the typed {@see ComplianceSdk} client as a singleton. Kept independent
 ...
 */
#[AsModule(name: 'ComplianceSdk', priority: 100)]
#[LoadsResources()]
final class ComplianceSdkServiceProvider extends ServiceProvider
{
    /**
     * The platform service this SDK talks to — the discovery discriminator.
     */
    private const string SERVICE = 'compliance';

    /**
     * Config namespace this SDK merges + reads.
     */
    private const string CONFIG_KEY = 'sdk.compliance';
    ...
    {
        $this->mergeConfigFrom(__DIR__ . '/../../config/compliance-sdk.php', self::CONFIG_KEY);

        $this->app->singleton(ComplianceSdk::class, function (Application $app): ComplianceSdk {
            ...
            if ((bool) $app->make('config')->get(self::CONFIG_KEY . '.fake', false)) {
                return new ComplianceSdk($registry);
            }
    ...
    {
        $this->publishes(
            [__DIR__ . '/../../config/compliance-sdk.php' => $this->app->configPath('compliance-sdk.php')],
            'compliance-sdk-config',
        );
    }

    /**
     * Construct this service's connector from its `sdk.compliance.*` config.
     */
