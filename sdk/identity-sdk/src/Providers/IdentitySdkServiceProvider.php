...
namespace Academorix\IdentitySdk\Providers;
...
/**
 * Root provider for `academorix/identity-sdk`.
 *
 * Builds the Identity service's own Saloon connector from `sdk.identity.*`,
 * runs a discovery pass scoped to `#[AsSdkResource(service: 'identity')]`, and
 * binds the typed {@see IdentitySdk} client as a singleton. Kept independent
 ...
 */
#[AsModule(name: 'IdentitySdk', priority: 100)]
#[LoadsResources()]
final class IdentitySdkServiceProvider extends ServiceProvider
{
    /**
     * The platform service this SDK talks to — the discovery discriminator.
     */
    private const string SERVICE = 'identity';

    /**
     * Config namespace this SDK merges + reads.
     */
    private const string CONFIG_KEY = 'sdk.identity';
    ...
    {
        $this->mergeConfigFrom(__DIR__ . '/../../config/identity-sdk.php', self::CONFIG_KEY);

        $this->app->singleton(IdentitySdk::class, function (Application $app): IdentitySdk {
            ...
            if ((bool) $app->make('config')->get(self::CONFIG_KEY . '.fake', false)) {
                return new IdentitySdk($registry);
            }
    ...
    {
        $this->publishes(
            [__DIR__ . '/../../config/identity-sdk.php' => $this->app->configPath('identity-sdk.php')],
            'identity-sdk-config',
        );
    }

    /**
     * Construct this service's connector from its `sdk.identity.*` config.
     */
