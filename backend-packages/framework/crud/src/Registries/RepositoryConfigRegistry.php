 ...
 * Repository Config Registry.
 *
 * Stores pre-resolved attribute configurations for repositories,
 * discovered at boot time via the composer-attribute-collector
 * manifest. Repositories read from this registry instead of
 * doing runtime reflection — same lookup table serves every
 * request on the worker.
 *
 * Bound as `#[Singleton]` per ADR 0006 — boot-populated tables
 * are immutable after discovery completes and safe to share.
 *
 * Supports lookup by:
