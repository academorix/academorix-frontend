...
@foreach ($rows as $row)
    ...
    @elseif($row['type'] === 'section')
        <div class="mx-1 flex">
            <span class="{{ $borderColor }}">{{ $frame['l'] }}</span>
            <span class="pl-1 text-stone-600">
                @foreach ($row['ancestors'] as $hasMore)
                    {{ $hasMore ? $pipe . $fsp . $fsp : $fsp . $fsp . $fsp }}
                @endforeach{{ $row['last'] ? $corner : $tee }}{{ $frame['t'] }}
            </span>
        ...
        <div class="mx-1 flex space-x-1">
            ...
            @if ($row['depth'] > 0)
                <span class="text-stone-600">
                    @foreach ($row['ancestors'] as $hasMore)
                        {{ $hasMore ? $pipe . $fsp . $fsp : $fsp . $fsp . $fsp }}
                    @endforeach{{ $row['last'] ? $corner : $tee }}
                </span>
