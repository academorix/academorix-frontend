...
namespace Academorix\BillingSdk\Client;

use Academorix\ApiSdk\Contracts\SdkResource;
use Academorix\ApiSdk\Registry\SdkResourceRegistry;

/**
 * Consumer-facing facade for the Billing service SDK.
 *
 * Every `$billing->{resource}()` call dispatches through `__call()` into the
 * service-scoped {@see SdkResourceRegistry} populated at boot by
 * {@see \Academorix\BillingSdk\Providers\BillingSdkServiceProvider}. Distinct
 ...
 */
final readonly class BillingSdk
{
    /**
    ...
    /**
     * Dispatch `$billing->{name}()` into the registry.
     *
    ...
    /**
     * Dispatch property-access (`$billing->{name}`) into the registry.
     */
