 ...
 * binds the typed {@see AccessSdk} client as a singleton. Kept independent
 ...
 */
#[AsModule(name: 'AccessSdk', priority: 100)]
#[LoadsResources()]
final class AccessSdkServiceProvider extends ServiceProvider
{
    /**
     * The platform service this SDK talks to — the discovery discriminator.
     */
    private const string SERVICE = 'access';

    /**
     * Config namespace this SDK merges + reads.
     */
    private const string CONFIG_KEY = 'sdk.access';
    ...
    {
        $this->mergeConfigFrom(__DIR__ . '/../../config/access-sdk.php', self::CONFIG_KEY);

        $this->app->singleton(AccessSdk::class, function (Application $app): AccessSdk {
    ...
    {
        $this->publishes(
            [__DIR__ . '/../../config/access-sdk.php' => $this->app->configPath('access-sdk.php')],
            'access-sdk-config',
        );
    }

    /**
     * Construct this service's connector from its `sdk.access.*` config.
     */
