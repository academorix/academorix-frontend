...
| **User**                       | `GET /api/v1/auth/me` (bootstrap)                                                            | ❌ **Missing — gap G1**            | Login DTO is minimal (`id,name,email,email_verified_at,status,last_login_at`); SPA needs roles/permissions/features/terminology/tenants/scopes. |
...
| `me` (identity bootstrap)                                                                                           | fixture                  | direct `httpClient.get('/v1/auth/me')` after login | Gap G1.                                                                            |
...
- Repurpose as the mock **bootstrap** response (`/api/v1/auth/me` — backend gap
...
**G1 — `GET /api/v1/auth/me` bootstrap endpoint** (tenant + platform surfaces)
...
Frontend POST /api/auth/login  { email, password, device_name: "Chrome on macOS 15.0" }
  ...
  ↓
Store token; call GET /api/v1/auth/me (backend gap G1)
  ↓
