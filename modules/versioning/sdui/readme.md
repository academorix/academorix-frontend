...
Platform-admin CRUD for the ApiVersion registry. Workspace users see a read-only projection under the same resource.
...
- `apiVersionsPublic` — the public / workspace read-only projection (excludes drafts + sunset).

## Broadcast subscriptions

None. Version lifecycle transitions are rare + workspace admins are notified via mail + in-app instead of live-updating dashboards.
