...
Human-in-loop work queue. Wraps a domain event (a lead needs a follow-up call,
a payment failed and someone should call the parent, a coach didn't submit
...
Per-tenant configurable rules (`when a lead sits >48h, create a follow-up task
for its owner`) authored via the `TaskAutomationRule` engine + `#[AsTaskAutomation]`
attribute on handler classes. Rules fire on domain events.

## Cross-references

- `growth/leads` — auto-creates follow-up tasks per stage.
- `finance/payment` — creates a task when a charge fails and dunning hits step 2.
