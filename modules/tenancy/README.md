...
`tenancy` sits directly on top of `foundation` and below every other module. It
...
container of tenants — a first-class tenancy concept. Foundation stays truly
...
| `BusinessType`      | config catalogue | Enum + config catalogue. Drives per-tenant defaults for features, terminology, roles, entitlements. Referenced by `tenants.business_type` (string column). Seeded from `data/business-types.json` into `config/tenancy.php`.                                                                              |
...
| `Branding`          | table            | Theme + logo profile per tenant. Multiple profiles per tenant (e.g. per-domain, dark / light). The Tenant row keeps a denormalised `branding` JSONB as a fallback for the workspace picker.                                                                                                               |
| `Identity`          | table            | Cross-tenant person. Application-scoped (no `tenant_id`), one Identity → many `Users` in different tenants. Solves the "invited to two workspaces" problem. Owned by `tenancy` because the linkage is a tenancy-boundary concept; the tenant-scoped `User` (in `user`) references this via `identity_id`. |
...
tenancy-owned exception is `Identity` — it is `application_id`-scoped because a
...
| `POST /api/v1/tenants/register`     | Self-serve workspace creation.           | throttled |
...
The tenant summary + workspace list are also embedded in `GET /api/auth/me` (see
...
- `foundation` — `Application` was previously scoped here but has moved to
  `tenancy`. Foundation still provides `HasSystemFlag`, `HasUserstamps`,
