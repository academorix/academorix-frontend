...
Per-workspace quota + usage tracking substrate. Wave 5 infrastructure. Consumed
by every metered feature module.
...
caller. Correctness wins over availability — an uncapped workspace is a billing
...
workspace + key + failure mode. Turn on only during declared incidents where
...
- Every fail-open window is documented in the incident retro with: duration,
  workspaces affected, total fail-open count, over-serve estimate in currency
  terms.
...
2. Applies the Workspace's `entitlement_overrides` JSONB (enterprise
   negotiations)
...
- **Doesn't own feature flags.** Feature flags are the `feature-flag` module (or
  Pennant). Entitlements are the ANSWER to "how much can this workspace do";
  feature flags are "can this workspace SEE this feature at all".
