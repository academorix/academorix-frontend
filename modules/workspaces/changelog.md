...
- **rename** — folder renamed `modules/Workspaces/` → `modules/workspaces/`;
  `README.md` → `readme.md`; every subdirectory lowercase. Enforces the
  workspace-wide kebab-case-lowercase rule.
- **drop** — `Academorix\Workspaces\Providers\WorkspacesRouteServiceProvider` removed
...
- **add: entities (+7)** — `Application`, `Domain`, `DomainRecord`, `Branding`,
  `Identity`, `WorkspaceContact`, `WorkspaceIntegration`. Rationale in
  `.kiro/product/analyses/module-blueprints-review.md` §2.2. `WorkspaceIdentity`
...
- **enrich: schemas** — every schema now carries `$version: 1`.
  `workspace.schema.json` gained `x-eloquent.baseClass`, `implements[]`,
  `appends[]`, `observers[]`.
- **enrich: traits.json** — added `owned[].schemaRef` and `consumed[].source`
  ...
  with the `->applicable()` migration macro for rows at the workspaces boundary.
- **split: branding** — extracted from `workspaces.branding` JSONB into its own
  entity + FK. The JSONB stays as a denormalised preview column on `Workspace` so
...
- Introduced under `modules/Workspaces/` with `Workspace` + `BusinessType` only, plus
...
- **drop** — `stancl/workspaces` composer dependency removed. Was pulled in purely
  for its `Domain` model contract + workspaces identification. We now ship our own
  `WorkspaceResolver` + `HostResolver` + `Academorix\Workspaces\Contracts\Domain`. Was
  overkill for our single-DB, row-level `workspace_id` architecture and added ~40
  unused classes.
- **drop: contracts** — `schemas/workspace.schema.json` no longer implements
  `Stancl\Workspaces\Contracts\Workspace` + `WorkspaceWithDatabase`.
  `schemas/domain.schema.json` no longer implements
  `Stancl\Workspaces\Contracts\Domain`.
...
- **add: trait** — `BelongsToWorkspaceOptional` published. Sibling of
  `BelongsToWorkspace` for rows where `workspace_id` may legitimately be NULL
  ...
  `->workspaceable_optional()`. Applied by `audit::Audit`,
  `notifications-mail::MailSuppression`, `notifications-sms::SmsOptOut`,
  `settings::SettingValue`.
- **add: events (+5)** — `WorkspaceErased` (GDPR Art. 17 cascade signal, fires
  before hard-delete so listeners can produce anonymisation entries),
  `WorkspaceUnsuspended` (distinct from Resumed \u2014 platform-admin action
  requires re-enabling integrations), `WorkspaceSettingsUpdated` (per-workspace
  summary distinct from per-field settings::SettingsChangeEvent),
  `IdentityErased` (cross-workspace Identity anonymisation when the last
...
- **add: listeners** — `WarmHostResolver` on `foundation::ApplicationBooted`
  ...
  `ConfigReloaded`, `MaybeEraseIdentity` + `PurgeWorkspaceContactsForErasedUser` on
  `user::UserErased`.
- **add: middleware** — `resolve.workspace.optional` for public-workspace read
...
- **note** — rename `workspaces` \u2192 `workspaces` is under active consideration
