...
- `Language` renamed → `PlatformLanguage`. Class, interface, observer
  ...
  (`PlatformLanguageController` on both workspace + platform-admin surfaces).
...
- Permission strings: `languages.viewAny`, `languages.view`,
  `platform.languages.*`.
- Event names: `LanguageEnabledForWorkspace`, `LanguageDisabledForWorkspace`.
- Error codes: `LANGUAGE_NOT_FOUND`, etc.
- Cache keys: `loc:languages:catalogue`.
- FK column names on downstream tables: `WorkspaceLocale.language_id`,
...
- Localization platform authored. Four entities: `Language`, `WorkspaceLocale`,
