...
`spatie/laravel-medialibrary` and adds the tenant-scope, entitlement, antivirus,
signed-URL, and chunked-upload layers Laravel Storage doesn't provide.
...
| ------------------------------------------------------------------------ | ---------------------------------------------------- |
| `File` Eloquent model with owner + tenant + kind + SHA-256 + virus state | Storage returns paths, not domain records            |
| Tenant-scoped path isolation on shared buckets                           | Storage doesn't know about tenants                   |
...
| Retention windows + cascade erase on tenant deletion                     | Storage doesn't run scheduled cleanup                |

Consumer modules NEVER call `Storage::disk()` directly. They compose the
`HasFiles` trait + declare `#[Attachable(kind: '...')]` and get tenant-aware
...
machine), and layer our tenant scoping + entitlement enforcement + antivirus
...
`tenant_id`, `owner_id` (nullable — system uploads), polymorphic
...
`BelongsToTenant`, `HasMetadata`, `HasUserstamps`, `HasAuditable`, `Filterable`,
`SoftDeletes`.
...
`tenant_id` (denormalised for scope queries), `variant_key` (`thumbnail` /
...
`tenant_id`, `issued_to_user_id` (nullable — anonymous share links), `reason`
...
`tenant_id`, `owner_id`, `upload_url`, `total_size_bytes`, `uploaded_bytes`,
...
- **share** — 7d (external sharing; requires tenant admin permission)
...
Revocation is a tenant admin action that writes `revoked_at` + emits
`SignedUrlRevoked`.
...
Every upload is hashed before commit. When a second tenant uploads a file
matching an existing blob's SHA-256:

- Both tenants get their own `File` rows (ownership + policy stay tenant-scoped)
- Both rows point at the same physical blob path
- Both consume bytes against their respective entitlements (each tenant pays for
  the reference)
...
Opt-out per kind via `#[FileKind(dedupable: false)]` — required when two tenants
MUST NOT share physical bytes even if content matches (cryptographically
sensitive files, signed contracts with tenant-specific DRM).
...
disk, `FileVirusFound` fired, `FileQuarantinedNotification` dispatched to tenant
owners)
...
- `storage.bytes.consumed` — pool, resets monthly OR at tenant anniversary
  (config), value = total bytes actively referenced by non-erased Files
- `storage.files.max_per_tenant` — slot, hard cap on File row count (rarely hit;
  primarily a defence against pathological upload floods)
