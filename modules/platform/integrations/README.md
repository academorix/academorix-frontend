...
- **Rule**: `valid_integration_provider` — checks that `provider` is a known
  key for the given `kind` (`config('integrations.providers')` whitelist).
...
without a KMS backend — PRODUCTION MUST OVERRIDE with a KMS-backed cipher
