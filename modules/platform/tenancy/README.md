...
- **Middleware** — `resolve.tenant`, `resolve.tenant.optional`, `tenant.user`,
  `platform.domain`.
- **Tenancy hooks** — `LogContextTenantHook` (priority 10), `CachePrefixTenantHook`
  (priority 20) — set per-tenant log context + cache prefix on request init.
...
- **Events** — 11 tenant lifecycle events (`TenantProvisioning`, `TenantProvisioned`,
  `TenantSuspended`, `TenantUnsuspended`, `TenantResumed`, `TenantArchived`,
  `TenantErased`, `TenantSettingsUpdated`, `TenantContactAdded/Updated/Removed`).
