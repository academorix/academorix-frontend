...
| Response cache (1h TTL, workspace-agnostic)                           | `geography.cache` middleware                                                                          |
...
Workspaces don't own reference-catalog rows. They express **preferences** via
settings:
...
These live in `workspace_settings` via the standard `settings` module contract.
...
- **Doesn't own workspace-scoped rows.** Reference-catalog data is global.
