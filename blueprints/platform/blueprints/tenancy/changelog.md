...
- **split: branding** — extracted from `tenants.branding` JSONB into its own
  ...
  the tenant picker does not need to join.
...
- **note** — rename `tenants` \u2192 `tenants` is under active consideration as
  a follow-up atomic refactor to align with the frontend's `TenantPicker` +
  `useMyTenancy` naming. Not part of this v0.2 commit. See settings-conversation
  notes.
