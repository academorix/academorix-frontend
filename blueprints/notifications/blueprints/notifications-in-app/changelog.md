...
- Two error codes: `NOTIFICATIONS_INAPP_BROADCAST_FAILED`,
  `NOTIFICATIONS_INAPP_REVERB_UNREACHABLE`.
- Kill switch: `notifications.in-app.broadcast` (default on). Off = DB-only
  writes.
- Health probe: `notifications-in-app-reverb-reachable`.

### Compatibility

- Depends on `notifications` (core) + `foundation` + `tenants`. No downstream
  extensions.
