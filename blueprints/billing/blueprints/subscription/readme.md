...
| Tenant's active subscription         | `Subscription` (wraps Cashier's model)                    |
...
└── Tenancy[]                                   ← tenants under this Application
      │
      ├── Cashier's Billable trait                 ← stores stripe_id / pm_type on tenants table
      │
      ├── Subscription (one active per tenant)  ← THIS MODULE'S entity
      ...
      └── Entitlement[]                             ← resolved by entitlements module
          ← Plan.default_entitlements + tenant.entitlement_overrides
```
...
- Chargebacks + refunds
- Customer portal (`$tenant->redirectToBillingPortal()`)
...
- Grace period logic beyond provider (7-day past_due banner → 14-day feature
  restrictions → 21-day full suspension)
- Tenant-state mapping (subscription state → tenant status)
...
- Enterprise contracts (annual PO invoices, offline processed — bypasses
  Cashier)

## 4. Provider selection per Application

Each `Application` has a `payment_provider` column set at creation. All tenants
under that Application use the same provider. The `SubscriptionServiceProvider`
boots the appropriate Cashier variant based on the resolved Application.
...
`Tenant` uses both trait bundles as a `union` — the active one resolved via
`CashierAdapter` binding at runtime based on
`Tenant::application->payment_provider`.

## 5. Grace period (our extension over provider default)

Stripe defaults to 3-day `past_due` → `canceled`. We extend the lifecycle:

| Days past due | Provider state                      | Our tenant state | User impact                                                |
| ------------: | ----------------------------------- | ---------------- | ---------------------------------------------------------- |
...
|         15–21 | `unpaid`                            | `suspended`      | Full tenant suspension (login OK, everything else refused) |
|           22+ | `canceled` (Stripe) or forced by us | `cancelled`      | Data retained 30d, then archive                            |

`AdvanceDunningStageJob` scans past_due subscriptions daily and progresses them
through the stages.

## 6. Enterprise contracts (offline billing)

Some tenants sign annual contracts paid via NET-30 PO invoices — not through
Stripe. We support this via `Plan.billing_mode = 'invoice'`:

- No Cashier `stripe_id` created
- `Subscription.provider = 'invoice'`
- `EnterpriseInvoice` model tracks PO number, invoice date, due date, paid
  status
- No payment_method attached; renewal is manual + confirmed via
...
- **Doesn't own the tenant lifecycle.** Tenant suspension/archival is tenancy
  module; we set flags via events.
