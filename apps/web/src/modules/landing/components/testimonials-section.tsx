 ...
 * @module modules/landing/components/testimonials-section
 *
 * @description
 * Three customer testimonials, each a semantic `<figure>` with a `<blockquote>`
 * and an attributed `<figcaption>`. Author avatars use initials fallbacks (no
 * remote image dependency), so the section renders crisply offline and in both
 * themes.
 */
...
interface Testimonial {
  /** The quoted text (without surrounding quotation marks). */
  quote: string;
  /** Author's full name. */
  name: string;
  /** Author's role and organisation. */
  role: string;
