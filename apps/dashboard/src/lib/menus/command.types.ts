 ...
 * `@/lib/menus` (not from `@/config/menu.config`). The re-export keeps a single
 ...
 * We re-export it here so a caller importing `@/lib/menus` has everything it
 ...
 * the `@/lib/menus` module should import these from here rather than from
 ...
 * from macOS Human Interface Guidelines and is reused by the context menu
 * grouping). Consumers should read this instead of hard-coding a category
 * order so a
...
// the tour. We re-export it here so consumers of `@/lib/menus` have a single
