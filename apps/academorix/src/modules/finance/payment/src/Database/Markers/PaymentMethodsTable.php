...
/**
 * @file Database/Markers/PaymentMethodsTable.php
 *
 * @description
 * Migration marker for the `payment_methods` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(PaymentMethodsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `payment_methods`. The resolver uses this as a tiebreaker among
...
final class PaymentMethodsTable
{
    ...
    public const string MIGRATION = '2026_07_15_120002_create_payment_methods_table.php';
