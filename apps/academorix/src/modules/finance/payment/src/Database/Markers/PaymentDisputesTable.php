...
/**
 * @file Database/Markers/PaymentDisputesTable.php
 *
 * @description
 * Migration marker for the `payment_disputes` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(PaymentDisputesTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `payment_disputes`. The resolver uses this as a tiebreaker among
...
namespace Academorix\Payment\Database\Markers;

use Stackra\Database\Attributes\DependsOn;

final class PaymentDisputesTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120000_create_payment_disputes_table.php';
