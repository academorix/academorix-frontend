...
/**
 * @file Database/Markers/PaymentIntentsTable.php
 *
 * @description
 * Migration marker for the `payment_intents` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(PaymentIntentsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `payment_intents`. The resolver uses this as a tiebreaker among
...
final class PaymentIntentsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120001_create_payment_intents_table.php';
