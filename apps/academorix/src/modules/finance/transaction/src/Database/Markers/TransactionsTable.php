...
/**
 * @file Database/Markers/TransactionsTable.php
 *
 * @description
 * Migration marker for the `transactions` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(TransactionsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `transactions`. The resolver uses this as a tiebreaker among
...
final class TransactionsTable
{
    ...
    public const string MIGRATION = '2026_07_15_120001_create_transactions_table.php';
