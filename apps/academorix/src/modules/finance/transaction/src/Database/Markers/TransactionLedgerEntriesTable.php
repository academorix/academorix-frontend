...
/**
 * @file Database/Markers/TransactionLedgerEntriesTable.php
 *
 * @description
 * Migration marker for the `transaction_ledger_entries` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(TransactionLedgerEntriesTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `transaction_ledger_entries`. The resolver uses this as a tiebreaker among
...
namespace Academorix\Transaction\Database\Markers;

use Stackra\Database\Attributes\DependsOn;

final class TransactionLedgerEntriesTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120000_create_transaction_ledger_entries_table.php';
