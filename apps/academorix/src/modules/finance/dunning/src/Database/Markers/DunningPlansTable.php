...
/**
 * @file Database/Markers/DunningPlansTable.php
 *
 * @description
 * Migration marker for the `dunning_plans` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(DunningPlansTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `dunning_plans`. The resolver uses this as a tiebreaker among
...
final class DunningPlansTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120001_create_dunning_plans_table.php';
