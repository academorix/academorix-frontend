...
/**
 * @file Database/Markers/DunningEventsTable.php
 *
 * @description
 * Migration marker for the `dunning_events` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(DunningEventsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `dunning_events`. The resolver uses this as a tiebreaker among
...
final class DunningEventsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120000_create_dunning_events_table.php';
