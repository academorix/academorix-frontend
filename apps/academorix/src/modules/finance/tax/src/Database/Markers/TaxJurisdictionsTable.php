...
/**
 * @file Database/Markers/TaxJurisdictionsTable.php
 *
 * @description
 * Migration marker for the `tax_jurisdictions` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(TaxJurisdictionsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `tax_jurisdictions`. The resolver uses this as a tiebreaker among
...
namespace Academorix\Tax\Database\Markers;

use Stackra\Database\Attributes\DependsOn;

final class TaxJurisdictionsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120002_create_tax_jurisdictions_table.php';
