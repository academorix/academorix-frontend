...
/**
 * @file Database/Markers/TaxRatesTable.php
 *
 * @description
 * Migration marker for the `tax_rates` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(TaxRatesTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `tax_rates`. The resolver uses this as a tiebreaker among
...
final class TaxRatesTable
{
    ...
    public const string MIGRATION = '2026_07_15_120003_create_tax_rates_table.php';
