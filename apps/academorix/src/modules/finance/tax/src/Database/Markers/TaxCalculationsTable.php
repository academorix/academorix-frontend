...
/**
 * @file Database/Markers/TaxCalculationsTable.php
 *
 * @description
 * Migration marker for the `tax_calculations` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(TaxCalculationsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `tax_calculations`. The resolver uses this as a tiebreaker among
...
final class TaxCalculationsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120000_create_tax_calculations_table.php';
