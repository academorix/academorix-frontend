...
/**
 * @file Database/Markers/TaxExemptionsTable.php
 *
 * @description
 * Migration marker for the `tax_exemptions` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(TaxExemptionsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `tax_exemptions`. The resolver uses this as a tiebreaker among
...
final class TaxExemptionsTable
{
    ...
    public const string MIGRATION = '2026_07_15_120001_create_tax_exemptions_table.php';
