...
/**
 * @file Database/Markers/FeeApplicationsTable.php
 *
 * @description
 * Migration marker for the `fee_applications` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(FeeApplicationsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `fee_applications`. The resolver uses this as a tiebreaker among
...
final class FeeApplicationsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120000_create_fee_applications_table.php';
