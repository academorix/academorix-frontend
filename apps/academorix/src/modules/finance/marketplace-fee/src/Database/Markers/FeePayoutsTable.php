...
/**
 * @file Database/Markers/FeePayoutsTable.php
 *
 * @description
 * Migration marker for the `fee_payouts` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(FeePayoutsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `fee_payouts`. The resolver uses this as a tiebreaker among
...
final class FeePayoutsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120001_create_fee_payouts_table.php';
