...
/**
 * @file Database/Markers/OrderLinesTable.php
 *
 * @description
 * Migration marker for the `order_lines` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(OrderLinesTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `order_lines`. The resolver uses this as a tiebreaker among
...
final class OrderLinesTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120001_create_order_lines_table.php';
