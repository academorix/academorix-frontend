...
/**
 * @file Database/Markers/CostCentersTable.php
 *
 * @description
 * Migration marker for the `cost_centers` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(CostCentersTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `cost_centers`. The resolver uses this as a tiebreaker among
...
final class CostCentersTable
{
    ...
    public const string MIGRATION = '2026_07_15_120001_create_cost_centers_table.php';
