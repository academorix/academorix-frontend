...
/**
 * @file Database/Markers/ExpenseCategoriesTable.php
 *
 * @description
 * Migration marker for the `expense_categories` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(ExpenseCategoriesTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `expense_categories`. The resolver uses this as a tiebreaker among
...
final class ExpenseCategoriesTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120002_create_expense_categories_table.php';
