...
/**
 * @file Database/Markers/PayrollLinesTable.php
 *
 * @description
 * Migration marker for the `payroll_lines` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(PayrollLinesTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `payroll_lines`. The resolver uses this as a tiebreaker among
...
final class PayrollLinesTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120004_create_payroll_lines_table.php';
