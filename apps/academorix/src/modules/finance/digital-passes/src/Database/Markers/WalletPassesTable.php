...
/**
 * @file Database/Markers/WalletPassesTable.php
 *
 * @description
 * Migration marker for the `wallet_passes` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(WalletPassesTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `wallet_passes`. The resolver uses this as a tiebreaker among
...
namespace Academorix\DigitalPasses\Database\Markers;

use Stackra\Database\Attributes\DependsOn;

final class WalletPassesTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120000_create_wallet_passes_table.php';
