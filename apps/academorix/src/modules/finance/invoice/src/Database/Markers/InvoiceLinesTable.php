...
/**
 * @file Database/Markers/InvoiceLinesTable.php
 *
 * @description
 * Migration marker for the `invoice_lines` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(InvoiceLinesTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `invoice_lines`. The resolver uses this as a tiebreaker among
...
namespace Academorix\Invoice\Database\Markers;

use Stackra\Database\Attributes\DependsOn;

final class InvoiceLinesTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120001_create_invoice_lines_table.php';
