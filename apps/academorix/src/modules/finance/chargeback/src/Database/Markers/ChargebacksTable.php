...
/**
 * @file Database/Markers/ChargebacksTable.php
 *
 * @description
 * Migration marker for the `chargebacks` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(ChargebacksTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `chargebacks`. The resolver uses this as a tiebreaker among
...
final class ChargebacksTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120001_create_chargebacks_table.php';
