...
/**
 * @file Database/Markers/ChargebackEvidencesTable.php
 *
 * @description
 * Migration marker for the `chargeback_evidences` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(ChargebackEvidencesTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `chargeback_evidences`. The resolver uses this as a tiebreaker among
...
namespace Academorix\Chargeback\Database\Markers;

use Stackra\Database\Attributes\DependsOn;

final class ChargebackEvidencesTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120000_create_chargeback_evidences_table.php';
