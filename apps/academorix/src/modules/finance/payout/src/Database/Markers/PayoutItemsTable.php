...
/**
 * @file Database/Markers/PayoutItemsTable.php
 *
 * @description
 * Migration marker for the `payout_items` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(PayoutItemsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `payout_items`. The resolver uses this as a tiebreaker among
...
final class PayoutItemsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120000_create_payout_items_table.php';
