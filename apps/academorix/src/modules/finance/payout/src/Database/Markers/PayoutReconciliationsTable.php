...
/**
 * @file Database/Markers/PayoutReconciliationsTable.php
 *
 * @description
 * Migration marker for the `payout_reconciliations` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(PayoutReconciliationsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `payout_reconciliations`. The resolver uses this as a tiebreaker among
...
namespace Academorix\Payout\Database\Markers;

use Stackra\Database\Attributes\DependsOn;

final class PayoutReconciliationsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120001_create_payout_reconciliations_table.php';
