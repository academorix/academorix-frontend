...
/**
 * @file Database/Markers/GatewayWebhookEventsTable.php
 *
 * @description
 * Migration marker for the `gateway_webhook_events` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(GatewayWebhookEventsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `gateway_webhook_events`. The resolver uses this as a tiebreaker among
...
final class GatewayWebhookEventsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120000_create_gateway_webhook_events_table.php';
