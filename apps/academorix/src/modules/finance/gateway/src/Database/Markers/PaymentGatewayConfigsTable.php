...
/**
 * @file Database/Markers/PaymentGatewayConfigsTable.php
 *
 * @description
 * Migration marker for the `payment_gateway_configs` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(PaymentGatewayConfigsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `payment_gateway_configs`. The resolver uses this as a tiebreaker among
...
namespace Academorix\Gateway\Database\Markers;

use Stackra\Database\Attributes\DependsOn;

final class PaymentGatewayConfigsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120001_create_payment_gateway_configs_table.php';
