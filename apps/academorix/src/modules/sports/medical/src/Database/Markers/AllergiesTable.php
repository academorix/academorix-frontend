...
/**
 * @file Database/Markers/AllergiesTable.php
 *
 * @description
 * Migration marker for the `allergies` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(AllergiesTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `allergies`. The resolver uses this as a tiebreaker among
...
final class AllergiesTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120000_create_allergies_table.php';

    /** Physical table name. */
    public const string TABLE = 'allergies';
}
