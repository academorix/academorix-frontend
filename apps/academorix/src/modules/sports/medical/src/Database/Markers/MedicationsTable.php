...
/**
 * @file Database/Markers/MedicationsTable.php
 *
 * @description
 * Migration marker for the `medications` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(MedicationsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `medications`. The resolver uses this as a tiebreaker among
...
final class MedicationsTable
{
    ...
    public const string MIGRATION = '2026_07_15_120004_create_medications_table.php';
