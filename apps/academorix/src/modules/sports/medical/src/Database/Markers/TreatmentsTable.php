...
/**
 * @file Database/Markers/TreatmentsTable.php
 *
 * @description
 * Migration marker for the `treatments` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(TreatmentsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `treatments`. The resolver uses this as a tiebreaker among
...
final class TreatmentsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120005_create_treatments_table.php';
