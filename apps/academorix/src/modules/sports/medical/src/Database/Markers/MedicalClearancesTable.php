...
/**
 * @file Database/Markers/MedicalClearancesTable.php
 *
 * @description
 * Migration marker for the `medical_clearances` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(MedicalClearancesTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `medical_clearances`. The resolver uses this as a tiebreaker among
...
namespace Academorix\Medical\Database\Markers;

use Stackra\Database\Attributes\DependsOn;

final class MedicalClearancesTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120002_create_medical_clearances_table.php';
