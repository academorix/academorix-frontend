...
/**
 * @file Database/Markers/PositionsTable.php
 *
 * @description
 * Migration marker for the `positions` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(PositionsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `positions`. The resolver uses this as a tiebreaker among
...
final class PositionsTable
{
    ...
    public const string MIGRATION = '2026_07_15_120001_create_positions_table.php';
