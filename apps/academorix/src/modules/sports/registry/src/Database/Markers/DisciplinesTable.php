...
/**
 * @file Database/Markers/DisciplinesTable.php
 *
 * @description
 * Migration marker for the `disciplines` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(DisciplinesTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `disciplines`. The resolver uses this as a tiebreaker among
...
namespace Academorix\Registry\Database\Markers;

use Stackra\Database\Attributes\DependsOn;

final class DisciplinesTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120000_create_disciplines_table.php';
