...
/**
 * @file Database/Markers/CoachAssignmentsTable.php
 *
 * @description
 * Migration marker for the `coach_assignments` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(CoachAssignmentsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `coach_assignments`. The resolver uses this as a tiebreaker among
...
final class CoachAssignmentsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120000_create_coach_assignments_table.php';
