...
/**
 * @file Database/Markers/CoachingProfilesTable.php
 *
 * @description
 * Migration marker for the `coaching_profiles` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(CoachingProfilesTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `coaching_profiles`. The resolver uses this as a tiebreaker among
...
final class CoachingProfilesTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120003_create_coaching_profiles_table.php';
