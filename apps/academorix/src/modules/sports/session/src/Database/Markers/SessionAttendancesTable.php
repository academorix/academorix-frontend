...
/**
 * @file Database/Markers/SessionAttendancesTable.php
 *
 * @description
 * Migration marker for the `session_attendances` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(SessionAttendancesTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `session_attendances`. The resolver uses this as a tiebreaker among
...
namespace Academorix\Session\Database\Markers;

use Stackra\Database\Attributes\DependsOn;

final class SessionAttendancesTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120000_create_session_attendances_table.php';
