...
/**
 * @file Database/Markers/GradingResultsTable.php
 *
 * @description
 * Migration marker for the `grading_results` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(GradingResultsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `grading_results`. The resolver uses this as a tiebreaker among
...
final class GradingResultsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120003_create_grading_results_table.php';
