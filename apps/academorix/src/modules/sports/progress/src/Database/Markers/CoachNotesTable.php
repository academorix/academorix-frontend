...
/**
 * @file Database/Markers/CoachNotesTable.php
 *
 * @description
 * Migration marker for the `coach_notes` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(CoachNotesTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `coach_notes`. The resolver uses this as a tiebreaker among
...
final class CoachNotesTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120001_create_coach_notes_table.php';

    /** Physical table name. */
    public const string TABLE = 'coach_notes';
}
