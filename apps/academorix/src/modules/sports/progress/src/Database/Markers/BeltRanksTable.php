...
/**
 * @file Database/Markers/BeltRanksTable.php
 *
 * @description
 * Migration marker for the `belt_ranks` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(BeltRanksTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `belt_ranks`. The resolver uses this as a tiebreaker among
...
final class BeltRanksTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120000_create_belt_ranks_table.php';
