...
/**
 * @file Database/Markers/ProgressCardsTable.php
 *
 * @description
 * Migration marker for the `progress_cards` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(ProgressCardsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `progress_cards`. The resolver uses this as a tiebreaker among
...
final class ProgressCardsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120005_create_progress_cards_table.php';
