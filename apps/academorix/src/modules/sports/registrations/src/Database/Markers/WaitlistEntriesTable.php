...
/**
 * @file Database/Markers/WaitlistEntriesTable.php
 *
 * @description
 * Migration marker for the `waitlist_entries` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(WaitlistEntriesTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `waitlist_entries`. The resolver uses this as a tiebreaker among
...
final class WaitlistEntriesTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120005_create_waitlist_entries_table.php';
