...
/**
 * @file Database/Markers/RegistrationActivitiesTable.php
 *
 * @description
 * Migration marker for the `registration_activities` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(RegistrationActivitiesTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `registration_activities`. The resolver uses this as a tiebreaker among
...
namespace Academorix\Registrations\Database\Markers;

use Stackra\Database\Attributes\DependsOn;

final class RegistrationActivitiesTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120001_create_registration_activities_table.php';
