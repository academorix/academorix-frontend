...
/**
 * @file Database/Markers/RegistrationTasksTable.php
 *
 * @description
 * Migration marker for the `registration_tasks` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(RegistrationTasksTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `registration_tasks`. The resolver uses this as a tiebreaker among
...
final class RegistrationTasksTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120002_create_registration_tasks_table.php';
