...
/**
 * @file Database/Markers/RegistrationsTable.php
 *
 * @description
 * Migration marker for the `registrations` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(RegistrationsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `registrations`. The resolver uses this as a tiebreaker among
...
final class RegistrationsTable
{
    ...
    public const string MIGRATION = '2026_07_15_120003_create_registrations_table.php';
