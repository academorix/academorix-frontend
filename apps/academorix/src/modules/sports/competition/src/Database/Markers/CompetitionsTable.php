...
/**
 * @file Database/Markers/CompetitionsTable.php
 *
 * @description
 * Migration marker for the `competitions` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(CompetitionsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `competitions`. The resolver uses this as a tiebreaker among
...
final class CompetitionsTable
{
    ...
    public const string MIGRATION = '2026_07_15_120003_create_competitions_table.php';
