...
/**
 * @file Database/Markers/CompetitionTeamsTable.php
 *
 * @description
 * Migration marker for the `competition_teams` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(CompetitionTeamsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `competition_teams`. The resolver uses this as a tiebreaker among
...
final class CompetitionTeamsTable
{
    ...
    public const string MIGRATION = '2026_07_15_120002_create_competition_teams_table.php';
