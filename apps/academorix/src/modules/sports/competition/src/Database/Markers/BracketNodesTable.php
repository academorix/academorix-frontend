...
/**
 * @file Database/Markers/BracketNodesTable.php
 *
 * @description
 * Migration marker for the `bracket_nodes` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(BracketNodesTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `bracket_nodes`. The resolver uses this as a tiebreaker among
...
final class BracketNodesTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120000_create_bracket_nodes_table.php';
