...
/**
 * @file Database/Markers/CompetitionFixturesTable.php
 *
 * @description
 * Migration marker for the `competition_fixtures` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(CompetitionFixturesTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `competition_fixtures`. The resolver uses this as a tiebreaker among
...
final class CompetitionFixturesTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120001_create_competition_fixtures_table.php';
