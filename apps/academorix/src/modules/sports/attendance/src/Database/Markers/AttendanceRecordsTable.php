...
/**
 * @file Database/Markers/AttendanceRecordsTable.php
 *
 * @description
 * Migration marker for the `attendance_records` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(AttendanceRecordsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `attendance_records`. The resolver uses this as a tiebreaker among
...
final class AttendanceRecordsTable
{
    ...
    public const string MIGRATION = '2026_07_15_120002_create_attendance_records_table.php';
