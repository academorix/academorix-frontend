...
/**
 * @file Database/Markers/SessionPlanItemsTable.php
 *
 * @description
 * Migration marker for the `session_plan_items` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(SessionPlanItemsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `session_plan_items`. The resolver uses this as a tiebreaker among
...
namespace Academorix\Drills\Database\Markers;

use Stackra\Database\Attributes\DependsOn;

final class SessionPlanItemsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120004_create_session_plan_items_table.php';
