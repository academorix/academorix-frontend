...
/**
 * @file Database/Markers/CurriculumWeeksTable.php
 *
 * @description
 * Migration marker for the `curriculum_weeks` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(CurriculumWeeksTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `curriculum_weeks`. The resolver uses this as a tiebreaker among
...
final class CurriculumWeeksTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120000_create_curriculum_weeks_table.php';
