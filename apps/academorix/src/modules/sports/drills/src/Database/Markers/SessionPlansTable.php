...
/**
 * @file Database/Markers/SessionPlansTable.php
 *
 * @description
 * Migration marker for the `session_plans` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(SessionPlansTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `session_plans`. The resolver uses this as a tiebreaker among
...
final class SessionPlansTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120005_create_session_plans_table.php';

    /** Physical table name. */
    public const string TABLE = 'session_plans';
}
