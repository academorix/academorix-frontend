...
/**
 * @file Database/Markers/DrillCategoriesTable.php
 *
 * @description
 * Migration marker for the `drill_categories` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(DrillCategoriesTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `drill_categories`. The resolver uses this as a tiebreaker among
...
final class DrillCategoriesTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120002_create_drill_categories_table.php';
