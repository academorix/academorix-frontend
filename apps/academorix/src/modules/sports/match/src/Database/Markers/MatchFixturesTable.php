...
/**
 * @file Database/Markers/MatchFixturesTable.php
 *
 * @description
 * Migration marker for the `match_fixtures` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(MatchFixturesTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `match_fixtures`. The resolver uses this as a tiebreaker among
...
final class MatchFixturesTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120001_create_match_fixtures_table.php';
