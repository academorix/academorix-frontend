...
/**
 * @file Database/Markers/MatchSquadEntriesTable.php
 *
 * @description
 * Migration marker for the `match_squad_entries` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(MatchSquadEntriesTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `match_squad_entries`. The resolver uses this as a tiebreaker among
...
namespace Academorix\Match\Database\Markers;

use Stackra\Database\Attributes\DependsOn;

final class MatchSquadEntriesTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120005_create_match_squad_entries_table.php';
