...
/**
 * @file Database/Markers/MatchNotesTable.php
 *
 * @description
 * Migration marker for the `match_notes` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(MatchNotesTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `match_notes`. The resolver uses this as a tiebreaker among
...
final class MatchNotesTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120002_create_match_notes_table.php';
