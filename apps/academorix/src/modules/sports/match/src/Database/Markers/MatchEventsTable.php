...
/**
 * @file Database/Markers/MatchEventsTable.php
 *
 * @description
 * Migration marker for the `match_events` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(MatchEventsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `match_events`. The resolver uses this as a tiebreaker among
...
final class MatchEventsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120000_create_match_events_table.php';
