...
/**
 * @file Database/Markers/OpponentLogosTable.php
 *
 * @description
 * Migration marker for the `opponent_logos` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(OpponentLogosTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `opponent_logos`. The resolver uses this as a tiebreaker among
...
final class OpponentLogosTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120006_create_opponent_logos_table.php';
