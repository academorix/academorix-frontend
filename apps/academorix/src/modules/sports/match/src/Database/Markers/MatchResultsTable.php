...
/**
 * @file Database/Markers/MatchResultsTable.php
 *
 * @description
 * Migration marker for the `match_results` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(MatchResultsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `match_results`. The resolver uses this as a tiebreaker among
...
final class MatchResultsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120004_create_match_results_table.php';
