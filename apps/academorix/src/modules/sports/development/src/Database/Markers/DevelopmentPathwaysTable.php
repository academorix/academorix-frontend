...
/**
 * @file Database/Markers/DevelopmentPathwaysTable.php
 *
 * @description
 * Migration marker for the `development_pathways` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(DevelopmentPathwaysTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `development_pathways`. The resolver uses this as a tiebreaker among
...
final class DevelopmentPathwaysTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120000_create_development_pathways_table.php';
