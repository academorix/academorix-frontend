...
/**
 * @file Database/Markers/CalendarSubscriptionsTable.php
 *
 * @description
 * Migration marker for the `calendar_subscriptions` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(CalendarSubscriptionsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `calendar_subscriptions`. The resolver uses this as a tiebreaker among
...
namespace Academorix\Event\Database\Markers;

use Stackra\Database\Attributes\DependsOn;

final class CalendarSubscriptionsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120000_create_calendar_subscriptions_table.php';
