...
/**
 * @file Database/Markers/EventInvitationsTable.php
 *
 * @description
 * Migration marker for the `event_invitations` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(EventInvitationsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `event_invitations`. The resolver uses this as a tiebreaker among
...
final class EventInvitationsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120002_create_event_invitations_table.php';
