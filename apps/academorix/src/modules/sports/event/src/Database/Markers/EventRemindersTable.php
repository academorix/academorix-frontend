...
/**
 * @file Database/Markers/EventRemindersTable.php
 *
 * @description
 * Migration marker for the `event_reminders` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(EventRemindersTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `event_reminders`. The resolver uses this as a tiebreaker among
...
final class EventRemindersTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120003_create_event_reminders_table.php';
