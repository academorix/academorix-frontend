...
/**
 * @file Database/Markers/AthleteEnrollmentsTable.php
 *
 * @description
 * Migration marker for the `athlete_enrollments` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(AthleteEnrollmentsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `athlete_enrollments`. The resolver uses this as a tiebreaker among
...
namespace Academorix\AthleteEnrollment\Database\Markers;

use Stackra\Database\Attributes\DependsOn;

final class AthleteEnrollmentsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120000_create_athlete_enrollments_table.php';
