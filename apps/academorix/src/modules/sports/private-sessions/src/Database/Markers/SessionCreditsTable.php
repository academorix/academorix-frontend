...
/**
 * @file Database/Markers/SessionCreditsTable.php
 *
 * @description
 * Migration marker for the `session_credits` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(SessionCreditsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `session_credits`. The resolver uses this as a tiebreaker among
...
final class SessionCreditsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120001_create_session_credits_table.php';
