...
/**
 * @file Database/Markers/TestBatteriesTable.php
 *
 * @description
 * Migration marker for the `test_batteries` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(TestBatteriesTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `test_batteries`. The resolver uses this as a tiebreaker among
...
final class TestBatteriesTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120003_create_test_batteries_table.php';
