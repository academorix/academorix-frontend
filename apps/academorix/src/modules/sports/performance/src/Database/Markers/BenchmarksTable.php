...
/**
 * @file Database/Markers/BenchmarksTable.php
 *
 * @description
 * Migration marker for the `benchmarks` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(BenchmarksTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `benchmarks`. The resolver uses this as a tiebreaker among
...
final class BenchmarksTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120000_create_benchmarks_table.php';
