...
/**
 * @file Database/Markers/PerformanceTestResultsTable.php
 *
 * @description
 * Migration marker for the `performance_test_results` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(PerformanceTestResultsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `performance_test_results`. The resolver uses this as a tiebreaker among
...
namespace Academorix\Performance\Database\Markers;

use Stackra\Database\Attributes\DependsOn;

final class PerformanceTestResultsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120001_create_performance_test_results_table.php';

    /** Physical table name. */
    public const string TABLE = 'performance_test_results';
}
