...
/**
 * @file Database/Markers/AnalyticsIdentitiesTable.php
 *
 * @description
 * Migration marker for the `analytics_identities` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(AnalyticsIdentitiesTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `analytics_identities`. The resolver uses this as a tiebreaker among
...
final class AnalyticsIdentitiesTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120002_create_analytics_identities_table.php';
