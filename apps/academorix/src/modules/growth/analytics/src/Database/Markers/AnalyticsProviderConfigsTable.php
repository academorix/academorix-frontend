...
/**
 * @file Database/Markers/AnalyticsProviderConfigsTable.php
 *
 * @description
 * Migration marker for the `analytics_provider_configs` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(AnalyticsProviderConfigsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `analytics_provider_configs`. The resolver uses this as a tiebreaker among
...
namespace Academorix\Analytics\Database\Markers;

use Stackra\Database\Attributes\DependsOn;

final class AnalyticsProviderConfigsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120003_create_analytics_provider_configs_table.php';
