...
/**
 * @file Database/Markers/AnalyticsDeliveriesTable.php
 *
 * @description
 * Migration marker for the `analytics_deliveries` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(AnalyticsDeliveriesTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `analytics_deliveries`. The resolver uses this as a tiebreaker among
...
final class AnalyticsDeliveriesTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120000_create_analytics_deliveries_table.php';
