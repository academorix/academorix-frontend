...
/**
 * @file Database/Markers/AnalyticsEventsTable.php
 *
 * @description
 * Migration marker for the `analytics_events` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(AnalyticsEventsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `analytics_events`. The resolver uses this as a tiebreaker among
...
final class AnalyticsEventsTable
{
    ...
    public const string MIGRATION = '2026_07_15_120001_create_analytics_events_table.php';
