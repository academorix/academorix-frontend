...
/**
 * @file Database/Markers/ReferralFraudFlagsTable.php
 *
 * @description
 * Migration marker for the `referral_fraud_flags` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(ReferralFraudFlagsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `referral_fraud_flags`. The resolver uses this as a tiebreaker among
...
namespace Academorix\Referrals\Database\Markers;

use Stackra\Database\Attributes\DependsOn;

final class ReferralFraudFlagsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120001_create_referral_fraud_flags_table.php';
