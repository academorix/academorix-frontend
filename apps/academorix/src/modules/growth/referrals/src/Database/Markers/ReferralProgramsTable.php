...
/**
 * @file Database/Markers/ReferralProgramsTable.php
 *
 * @description
 * Migration marker for the `referral_programs` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(ReferralProgramsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `referral_programs`. The resolver uses this as a tiebreaker among
...
final class ReferralProgramsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120002_create_referral_programs_table.php';
