...
/**
 * @file Database/Markers/LeadTasksTable.php
 *
 * @description
 * Migration marker for the `lead_tasks` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(LeadTasksTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `lead_tasks`. The resolver uses this as a tiebreaker among
...
final class LeadTasksTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120001_create_lead_tasks_table.php';
