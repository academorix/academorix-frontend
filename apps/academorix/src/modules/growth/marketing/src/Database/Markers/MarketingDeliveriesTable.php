...
/**
 * @file Database/Markers/MarketingDeliveriesTable.php
 *
 * @description
 * Migration marker for the `marketing_deliveries` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(MarketingDeliveriesTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `marketing_deliveries`. The resolver uses this as a tiebreaker among
...
final class MarketingDeliveriesTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120001_create_marketing_deliveries_table.php';
