...
/**
 * @file Database/Markers/MarketingDeadLettersTable.php
 *
 * @description
 * Migration marker for the `marketing_dead_letters` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(MarketingDeadLettersTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `marketing_dead_letters`. The resolver uses this as a tiebreaker among
...
final class MarketingDeadLettersTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120000_create_marketing_dead_letters_table.php';
