...
/**
 * @file Database/Markers/MarketingEventsTable.php
 *
 * @description
 * Migration marker for the `marketing_events` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(MarketingEventsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `marketing_events`. The resolver uses this as a tiebreaker among
...
final class MarketingEventsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120002_create_marketing_events_table.php';
