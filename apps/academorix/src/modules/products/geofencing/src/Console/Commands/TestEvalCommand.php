...
final class TestEvalCommand extends BaseCommand
{
     ...
     * @var string
     */
    protected $description = 'Smoke-test one geofence evaluation.';
    ...
    public function handle(GeofenceServiceInterface $service): int
    {
        ...
        if ($className === null || ! \class_exists($className)) {
            $this->error("Unknown fenceable_type alias: {$fenceableType}");
        ...
        if (! $row instanceof Geofenceable) {
            $this->error("Fenceable {$fenceableType}:{$fenceableId} not found or not Geofenceable.");
