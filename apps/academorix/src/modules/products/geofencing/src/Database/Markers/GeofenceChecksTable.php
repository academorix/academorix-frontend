...
/**
 * @file Database/Markers/GeofenceChecksTable.php
 *
 * @description
 * Migration marker for the `geofence_checks` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(GeofenceChecksTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `geofence_checks`. The resolver uses this as a tiebreaker among
...
namespace Academorix\Geofencing\Database\Markers;

use Stackra\Database\Attributes\DependsOn;

final class GeofenceChecksTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_000200_create_geofence_checks_table.php';
