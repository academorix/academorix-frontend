...
/**
 * @file Database/Markers/PolicyAcknowledgementsTable.php
 *
 * @description
 * Migration marker for the `policy_acknowledgements` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(PolicyAcknowledgementsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `policy_acknowledgements`. The resolver uses this as a tiebreaker among
...
namespace Academorix\Safeguarding\Database\Markers;

use Stackra\Database\Attributes\DependsOn;

final class PolicyAcknowledgementsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120001_create_policy_acknowledgements_table.php';
