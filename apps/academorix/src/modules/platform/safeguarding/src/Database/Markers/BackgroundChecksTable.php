...
/**
 * @file Database/Markers/BackgroundChecksTable.php
 *
 * @description
 * Migration marker for the `background_checks` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(BackgroundChecksTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `background_checks`. The resolver uses this as a tiebreaker among
...
final class BackgroundChecksTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120000_create_background_checks_table.php';
