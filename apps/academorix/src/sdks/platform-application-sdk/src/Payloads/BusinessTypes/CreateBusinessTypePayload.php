 ...
 * service seeds a matching row into `config/tenants.php` +
 * `data/business-types.json` on save; a deploy is NOT required for
 * new keys to become bindable on Tenant rows.
 *
 * ## Constraints
 *
...
{
    /**
     * @param  string                    $key            Enum key — also the value written to `tenants.business_type`. Snake-case identifier.
     * @param  string                    $label          Active-locale human label.
     * @param  string                    $description    Active-locale description.
     * @param  array<string, mixed>      $defaultConfig  Bag applied at tenant provisioning time (features / terminology / roles / entitlements / onboarding steps / sports).
