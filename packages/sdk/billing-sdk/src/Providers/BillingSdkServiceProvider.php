 ...
 * binds the typed {@see BillingSdk} client as a singleton. Kept independent
 ...
 */
#[AsModule(name: 'BillingSdk', priority: 100)]
#[LoadsResources()]
final class BillingSdkServiceProvider extends ServiceProvider
{
    /**
     * The platform service this SDK talks to — the discovery discriminator.
     */
    private const string SERVICE = 'billing';

    /**
     * Config namespace this SDK merges + reads.
     */
    private const string CONFIG_KEY = 'sdk.billing';
    ...
    {
        $this->mergeConfigFrom(__DIR__ . '/../../config/billing-sdk.php', self::CONFIG_KEY);

        $this->app->singleton(BillingSdk::class, function (Application $app): BillingSdk {
            ...
            if ((bool) $app->make('config')->get(self::CONFIG_KEY . '.fake', false)) {
                return new BillingSdk($registry);
            }
    ...
    {
        $this->publishes(
            [__DIR__ . '/../../config/billing-sdk.php' => $this->app->configPath('billing-sdk.php')],
            'billing-sdk-config',
        );
    }

    /**
     * Construct this service's connector from its `sdk.billing.*` config.
     */
