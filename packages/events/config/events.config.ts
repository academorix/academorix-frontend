 ...
 * @description Consumer template for the events module's namespaced
 *   configuration factory.
 *
 *   Copy this file into your app's `src/config/` directory and
 *   customise. The single call to `registerAs('events', ...)` from
 *   `@stackra/config` binds the return value under a namespaced DI
 *   token (`eventsConfig.KEY`) and attaches an `.asProvider()` helper
 *   for `EventEmitterModule.forRootAsync(...)`.
 *
 * @example
 * ```typescript
