 ...
 *   `import { defineConfig } from '@stackra/events'` toward
 ...
 *   bound to `IEventEmitterConfig`), so every existing call site
 ...
 *
 * Identity function bound to {@link IEventEmitterConfig}. Preserves
 ...
 *   export default defineConfig({ wildcard: true, delimiter: '.' });
 *
 *   // After
 *   import { registerAs } from '@stackra/config';
 ...
 *   ```
 *
 * @typeParam T - Constrained to `IEventEmitterConfig` so the caller
 *   literal narrows past the interface widening.
 * @param config - Events module configuration object.
 * @returns The same config object, unchanged.
 */
export function defineConfig<T extends IEventEmitterConfig>(config: T): T {
  ...
  if (!warned) {
    ...
    if (typeof console !== "undefined" && typeof console.warn === "function") {
      console.warn(
        "[@stackra/events] defineConfig is deprecated and will be removed in v0.2. " +
...
// `import { registerAs } from '@stackra/events'` for one release
