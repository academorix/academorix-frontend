 ...
 *   `import { defineConfig } from '@stackra/logger'` toward
 ...
 *   bound to `ILoggerModuleConfig`), so every existing call site
 ...
 *
 * Identity function bound to {@link ILoggerModuleConfig}. Preserves
 ...
 *   export default defineConfig({ default: 'app', channels: {...} });
 *
 *   // After
 *   import { registerAs, env } from '@stackra/config';
 *   export const loggerConfig = registerAs<ILoggerModuleConfig>('logger', () => ({
 *     default: env('LOG_CHANNEL', 'app'),
 *     channels: { app: { driver: 'single', level: env('LOG_LEVEL', 'info') } },
 *   }));
 *   ```
 *
 * @typeParam T - Constrained to `ILoggerModuleConfig` so the caller
 ...
 */
export function defineConfig<T extends ILoggerModuleConfig>(config: T): T {
  ...
  if (!warned) {
    ...
    if (typeof console !== "undefined" && typeof console.warn === "function") {
      console.warn(
        "[@stackra/logger] defineConfig is deprecated and will be removed in v0.2. " +
...
// `import { registerAs } from '@stackra/logger'` for one release
