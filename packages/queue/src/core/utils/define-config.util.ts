 ...
 *   `import { defineConfig } from '@stackra/queue'` toward
 ...
 *   bound to `IQueueModuleOptions`), so every existing call site
 ...
 *
 * Identity function bound to {@link IQueueModuleOptions}. Preserves
 ...
 *   export default defineConfig({ default: 'memory', connections: {...} });
 *
 *   // After
 ...
 *   ```
 *
 * @typeParam T - Constrained to `IQueueModuleOptions` so the caller
 *   literal narrows past the interface widening.
 * @param config - Queue module configuration object.
 * @returns The same config object, unchanged.
 */
export function defineConfig<T extends IQueueModuleOptions>(config: T): T {
  ...
  if (!warned) {
    ...
    if (typeof console !== "undefined" && typeof console.warn === "function") {
      console.warn(
        "[@stackra/queue] defineConfig is deprecated and will be removed in v0.2. " +
...
// `import { registerAs } from '@stackra/queue'` for one release
