...
/**
 * @file Database/Markers/LegalHoldsTable.php
 *
 * @description
 * Migration marker for the `legal_holds` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(LegalHoldsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `legal_holds`. The resolver uses this as a tiebreaker among
...
final class LegalHoldsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_000304_create_legal_holds_table.php';
