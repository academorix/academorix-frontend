...
/**
 * @file Database/Markers/SubprocessorsTable.php
 *
 * @description
 * Migration marker for the `subprocessors` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(SubprocessorsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `subprocessors`. The resolver uses this as a tiebreaker among
...
final class SubprocessorsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_000306_create_subprocessors_table.php';
