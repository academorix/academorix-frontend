...
/**
 * @file Database/Markers/ConsentRecordsTable.php
 *
 * @description
 * Migration marker for the `consent_records` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(ConsentRecordsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `consent_records`. The resolver uses this as a tiebreaker among
...
final class ConsentRecordsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_000301_create_consent_records_table.php';
