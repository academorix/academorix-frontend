...
/**
 * @file Database/Markers/ConsentCategoriesTable.php
 *
 * @description
 * Migration marker for the `consent_categories` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(ConsentCategoriesTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `consent_categories`. The resolver uses this as a tiebreaker among
...
final class ConsentCategoriesTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_000300_create_consent_categories_table.php';
