...
/**
 * @file Database/Markers/RetentionRunsTable.php
 *
 * @description
 * Migration marker for the `retention_runs` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(RetentionRunsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `retention_runs`. The resolver uses this as a tiebreaker among
...
final class RetentionRunsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_000305_create_retention_runs_table.php';
