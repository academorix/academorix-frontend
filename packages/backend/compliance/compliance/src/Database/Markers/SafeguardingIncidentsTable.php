...
/**
 * @file Database/Markers/SafeguardingIncidentsTable.php
 *
 * @description
 * Migration marker for the `safeguarding_incidents` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(SafeguardingIncidentsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `safeguarding_incidents`. The resolver uses this as a tiebreaker among
...
namespace Stackra\Compliance\Database\Markers;

use Stackra\Database\Attributes\DependsOn;

final class SafeguardingIncidentsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_000307_create_safeguarding_incidents_table.php';
