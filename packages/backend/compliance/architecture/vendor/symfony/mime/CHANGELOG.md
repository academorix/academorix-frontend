...
- Replace `__sleep/wakeup()` by `__(un)serialize()` on `AbstractPart`
  implementations

7.4
---

- Deprecate implementing `__sleep/wakeup()` on `AbstractPart` implementations;
  use `__(un)serialize()` instead
