...
- Add `localeLower()`, `localeUpper()`, `localeTitle()` methods to
...
- added the `s()` helper method to get either an `UnicodeString` or `ByteString`
  instance, depending of the input string UTF-8 compliance
- added `$cut` parameter to
  `Symfony\Component\String\AbstractString::truncate()`
