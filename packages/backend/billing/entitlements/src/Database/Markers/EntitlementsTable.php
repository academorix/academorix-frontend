...
/**
 * @file Database/Markers/EntitlementsTable.php
 *
 * @description
 * Migration marker for the `entitlements` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(EntitlementsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `entitlements`. The resolver uses this as a tiebreaker among
...
final class EntitlementsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_000110_create_entitlements_table.php';
