...
/**
 * @file Database/Markers/EntitlementUsagesTable.php
 *
 * @description
 * Migration marker for the `entitlement_usages` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(EntitlementUsagesTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `entitlement_usages`. The resolver uses this as a tiebreaker among
...
namespace Stackra\Entitlements\Database\Markers;

use Stackra\Database\Attributes\DependsOn;

final class EntitlementUsagesTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_000111_create_entitlement_usages_table.php';
