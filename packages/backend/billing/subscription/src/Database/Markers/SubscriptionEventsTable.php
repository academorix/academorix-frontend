...
/**
 * @file Database/Markers/SubscriptionEventsTable.php
 *
 * @description
 * Migration marker for the `subscription_events` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(SubscriptionEventsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `subscription_events`. The resolver uses this as a tiebreaker among
...
namespace Stackra\Subscription\Database\Markers;

use Stackra\Database\Attributes\DependsOn;

final class SubscriptionEventsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_000202_create_subscription_events_table.php';
