...
/**
 * @file Database/Markers/SubscriptionsTable.php
 *
 * @description
 * Migration marker for the `subscriptions` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(SubscriptionsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `subscriptions`. The resolver uses this as a tiebreaker among
...
final class SubscriptionsTable
{
    ...
    public const string MIGRATION = '2026_07_15_000201_create_subscriptions_table.php';
