...
Demonstrates simple one-line status messages and detailed status blocks with
title, details, and help text.

### Data Tables

Shows how to create formatted key-value tables with headers, basic rows, and
status indicator rows (success, error, warning, etc.).

### Visual Elements

Displays rounded and square boxes with custom colors, plus all horizontal rule
variants.
...
Shows async task execution with different outcomes (success, warning, error) and
...
Shows how to use `render()`, `liveRender()`, and `terminal()` without the
OmniTerm trait.
