...
- Custom `Application` class extending Laravel's Application with `App\`
  namespace
...
- `GET /api/{version}` - Returns 401 (unauthorized) where version matches
  `v[0-9]+` (e.g., v1, v2, v10)
...
The foundation module automatically registers routes and provides the custom
Application class. No additional configuration is required.
