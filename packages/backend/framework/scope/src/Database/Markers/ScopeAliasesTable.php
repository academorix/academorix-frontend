...
/**
 * @file Database/Markers/ScopeAliasesTable.php
 *
 * @description
 * Migration marker for the `scope_aliases` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(ScopeAliasesTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `scope_aliases`. The resolver uses this as a tiebreaker among
...
final class ScopeAliasesTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_11_000004_create_scope_aliases_table.php';
