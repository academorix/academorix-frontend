...
/**
 * @file Database/Markers/ScopeValuesTable.php
 *
 * @description
 * Migration marker for the `scope_values` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(ScopeValuesTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `scope_values`. The resolver uses this as a tiebreaker among
...
final class ScopeValuesTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_11_000003_create_scope_values_table.php';
