...
/**
 * @file Database/Markers/ScopeNodesTable.php
 *
 * @description
 * Migration marker for the `scope_nodes` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(ScopeNodesTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `scope_nodes`. The resolver uses this as a tiebreaker among
...
final class ScopeNodesTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_11_000002_create_scope_nodes_table.php';
