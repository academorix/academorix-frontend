...
- [#90](https://github.com/sebastianbergmann/exporter/issues/90):
  `Exporter::toArray()` silently drops a private property that is redeclared in
  a derived class
...
- Suppress `unexpected NAN value was coerced to string` warning triggered on PHP
  8.5
...
- Suppress `not representable as an int, cast occurred` warning triggered on PHP
  8.5
