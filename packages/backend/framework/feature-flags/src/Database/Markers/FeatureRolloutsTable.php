...
/**
 * @file Database/Markers/FeatureRolloutsTable.php
 *
 * @description
 * Migration marker for the `feature_rollouts` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(FeatureRolloutsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `feature_rollouts`. The resolver uses this as a tiebreaker among
...
final class FeatureRolloutsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2025_11_01_000003_create_feature_rollouts_table.php';
