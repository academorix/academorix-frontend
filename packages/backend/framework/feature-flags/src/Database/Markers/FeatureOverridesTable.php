...
/**
 * @file Database/Markers/FeatureOverridesTable.php
 *
 * @description
 * Migration marker for the `feature_overrides` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(FeatureOverridesTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `feature_overrides`. The resolver uses this as a tiebreaker among
...
final class FeatureOverridesTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2025_11_01_000002_create_feature_overrides_table.php';
