...
- **Delegation** — user X grants user Y one of X's roles for a bounded window.
  Y's tokens carry the delegated role until the window closes.
...
- `DelegationEnforcer` — extends the RBAC PermissionResolver: when a delegation
  is active for the caller, resolve the delegated role's permissions on top of
  the caller's own.
- `ImpersonationSession` — issue + revoke helpers. On impersonation start, every
  issued JWT carries `purpose = impersonation` + an `impersonator` claim
  (platform_user_id + reason).
