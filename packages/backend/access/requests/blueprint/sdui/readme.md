...
Tenant-facing self-service surface. The most-used files are the list (both "my
requests" and "pending my review") and the show detail. The `create` screen is
rarely opened directly \u2014 the primary path is the `AccessDeniedPanel` on the
403 screen, which pre-fills the form.
...
- `accessRequests` \u2014 paginated list (`GET /api/v1/access-requests` with
...
- `requestableResources` \u2014 map of registered `#[AccessRequestable]` types
  ...
  (`GET /api/v1/access-requests/requestable-resources`).
- `myPermissions` \u2014 caller's currently-held permissions (`GET /api/auth/me`
  \u2192 `identity.permissions`). Powers the "already have this" short-circuit
  on the create form.
...
- `tenant.{tenantId}.access-requests` \u2014 tenant admin dashboard sees live
  state.
- `user.{userId}.access-requests` \u2014 requester + approver see their own
  inbox updates.
...
reads `resource`, `permissions_wanted`, and `request_payload_template` from the
403 body, renders the "you don't have access" copy + a big "Request access"
button, and \u2014 when pressed \u2014 opens
