...
/**
 * @file Database/Markers/InvitationEventsTable.php
 *
 * @description
 * Migration marker for the `invitation_events` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(InvitationEventsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `invitation_events`. The resolver uses this as a tiebreaker among
...
namespace Stackra\Invitations\Database\Markers;

use Stackra\Database\Attributes\DependsOn;

final class InvitationEventsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_000131_create_invitation_events_table.php';
