...
/**
 * @file Database/Markers/InvitationsTable.php
 *
 * @description
 * Migration marker for the `invitations` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(InvitationsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `invitations`. The resolver uses this as a tiebreaker among
...
final class InvitationsTable
{
    ...
    public const string MIGRATION = '2026_07_15_000130_create_invitations_table.php';
