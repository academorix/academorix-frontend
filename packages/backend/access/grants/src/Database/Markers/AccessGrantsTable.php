...
/**
 * @file Database/Markers/AccessGrantsTable.php
 *
 * @description
 * Migration marker for the `access_grants` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(AccessGrantsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `access_grants`. The resolver uses this as a tiebreaker among
...
namespace Stackra\Grants\Database\Markers;

use Stackra\Database\Attributes\DependsOn;

final class AccessGrantsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120000_create_access_grants_table.php';
