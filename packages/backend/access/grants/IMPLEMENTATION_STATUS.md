...
Arbitrary per-user grants beyond roles — a discretionary permission attached to
a User for a specific resource + scope, bounded by an optional expiry. Sits
...
- `GrantResolver` — for a (user, ability, resource) tuple, resolve to a bool by
  checking active + non-expired + matching grantable. Called by the
...
- `GrantableRegistry` — attribute-discovered map of `grantable_type` → concrete
  grantable resolver. Every module that wants to be a grantable target ships a
  class with `#[AsGrantable]` naming the ability namespace.
