...
/**
 * @file Database/Markers/PermissionsTable.php
 *
 * @description
 * Migration marker for the `permissions` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(PermissionsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `permissions`. The resolver uses this as a tiebreaker among
...
final class PermissionsTable
{
    ...
    public const string MIGRATION = '2026_07_15_120002_create_permissions_table.php';
