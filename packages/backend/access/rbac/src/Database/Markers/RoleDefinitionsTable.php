...
/**
 * @file Database/Markers/RoleDefinitionsTable.php
 *
 * @description
 * Migration marker for the `role_definitions` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(RoleDefinitionsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `role_definitions`. The resolver uses this as a tiebreaker among
...
final class RoleDefinitionsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120003_create_role_definitions_table.php';
