...
/**
 * @file Database/Markers/ModelHasPermissionsesTable.php
 *
 * @description
 * Migration marker for the `model_has_permissionses` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(ModelHasPermissionsesTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `model_has_permissionses`. The resolver uses this as a tiebreaker among
...
namespace Stackra\Rbac\Database\Markers;

use Stackra\Database\Attributes\DependsOn;

final class ModelHasPermissionsesTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120000_create_model_has_permissionses_table.php';
