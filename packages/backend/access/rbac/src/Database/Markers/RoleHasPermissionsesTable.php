...
/**
 * @file Database/Markers/RoleHasPermissionsesTable.php
 *
 * @description
 * Migration marker for the `role_has_permissionses` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(RoleHasPermissionsesTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `role_has_permissionses`. The resolver uses this as a tiebreaker among
...
final class RoleHasPermissionsesTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120004_create_role_has_permissionses_table.php';
