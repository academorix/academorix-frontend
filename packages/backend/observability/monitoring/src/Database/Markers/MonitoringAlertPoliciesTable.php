...
/**
 * @file Database/Markers/MonitoringAlertPoliciesTable.php
 *
 * @description
 * Migration marker for the `monitoring_alert_policies` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(MonitoringAlertPoliciesTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `monitoring_alert_policies`. The resolver uses this as a tiebreaker among
...
final class MonitoringAlertPoliciesTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120002_create_monitoring_alert_policies_table.php';
