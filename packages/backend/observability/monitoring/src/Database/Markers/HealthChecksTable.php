...
/**
 * @file Database/Markers/HealthChecksTable.php
 *
 * @description
 * Migration marker for the `health_checks` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(HealthChecksTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `health_checks`. The resolver uses this as a tiebreaker among
...
final class HealthChecksTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120001_create_health_checks_table.php';
