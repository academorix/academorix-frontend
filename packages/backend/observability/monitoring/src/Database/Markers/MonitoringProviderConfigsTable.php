...
/**
 * @file Database/Markers/MonitoringProviderConfigsTable.php
 *
 * @description
 * Migration marker for the `monitoring_provider_configs` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(MonitoringProviderConfigsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `monitoring_provider_configs`. The resolver uses this as a tiebreaker among
...
namespace Stackra\Monitoring\Database\Markers;

use Stackra\Database\Attributes\DependsOn;

final class MonitoringProviderConfigsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120005_create_monitoring_provider_configs_table.php';
