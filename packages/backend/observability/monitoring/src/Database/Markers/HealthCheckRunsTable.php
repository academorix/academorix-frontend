...
/**
 * @file Database/Markers/HealthCheckRunsTable.php
 *
 * @description
 * Migration marker for the `health_check_runs` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(HealthCheckRunsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `health_check_runs`. The resolver uses this as a tiebreaker among
...
final class HealthCheckRunsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120000_create_health_check_runs_table.php';
