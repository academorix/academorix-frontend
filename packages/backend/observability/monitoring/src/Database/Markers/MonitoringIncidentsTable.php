...
/**
 * @file Database/Markers/MonitoringIncidentsTable.php
 *
 * @description
 * Migration marker for the `monitoring_incidents` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(MonitoringIncidentsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `monitoring_incidents`. The resolver uses this as a tiebreaker among
...
final class MonitoringIncidentsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120004_create_monitoring_incidents_table.php';
