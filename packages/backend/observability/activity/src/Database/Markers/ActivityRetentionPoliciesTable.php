...
/**
 * @file Database/Markers/ActivityRetentionPoliciesTable.php
 *
 * @description
 * Migration marker for the `activity_retention_policies` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(ActivityRetentionPoliciesTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `activity_retention_policies`. The resolver uses this as a tiebreaker among
...
namespace Stackra\Activity\Database\Markers;

use Stackra\Database\Attributes\DependsOn;

final class ActivityRetentionPoliciesTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120000_create_activity_retention_policies_table.php';
