...
1. **Tax rates** — per-region per-jurisdiction rates (US: state + county; EU:
   country VAT; GCC: country + emirate).
...
- `ComputeTaxAction` — POST /tax/compute — synchronous compute for a proposed
  line. Used by cart preview.
...
- `ValidateExemptionCertificateAction` — POST /tax/exemptions/{cert}/validate —
  admin-triggered validation (some jurisdictions require verification against a
  state database).

### Support services

- `TaxRateResolver` (Services/) — given a (line, jurisdiction) pair, returns the
  applicable `TaxRate` row. Handles overlapping rate rules (US state + county +
  city; Canadian GST + PST).
...
- `AvalaraAdapter` (Services/, OPTIONAL) — for enterprise tenants with a real
  tax engine subscription. Falls back to the internal `TaxCalculator` when not
  configured.
...
domain-specific actions. Follow the coupon pattern: standard CRUD delegates to
`RepositoryInterface`, domain actions delegate to `TaxCalculator` /
`TaxReporter`.
