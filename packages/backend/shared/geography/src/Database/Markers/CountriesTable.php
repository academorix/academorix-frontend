...
/**
 * @file Database/Markers/CountriesTable.php
 *
 * @description
 * Migration marker for the `countries` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(CountriesTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `countries`. The resolver uses this as a tiebreaker among
...
final class CountriesTable
{
    ...
    public const string MIGRATION = '2026_07_15_120001_create_countries_table.php';
