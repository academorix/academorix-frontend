...
/**
 * @file Database/Markers/TimezonesTable.php
 *
 * @description
 * Migration marker for the `timezones` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(TimezonesTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `timezones`. The resolver uses this as a tiebreaker among
...
namespace Stackra\Geography\Database\Markers;

use Stackra\Database\Attributes\DependsOn;

final class TimezonesTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120005_create_timezones_table.php';
