...
/**
 * @file Database/Markers/CurrenciesTable.php
 *
 * @description
 * Migration marker for the `currencies` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(CurrenciesTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `currencies`. The resolver uses this as a tiebreaker among
...
final class CurrenciesTable
{
    ...
    public const string MIGRATION = '2026_07_15_120002_create_currencies_table.php';
