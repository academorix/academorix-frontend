...
/**
 * @file Database/Markers/SyncCursorsTable.php
 *
 * @description
 * Migration marker for the `sync_cursors` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(SyncCursorsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `sync_cursors`. The resolver uses this as a tiebreaker among
...
namespace Stackra\OfflineSync\Database\Markers;

use Stackra\Database\Attributes\DependsOn;

final class SyncCursorsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120000_create_sync_cursors_table.php';
