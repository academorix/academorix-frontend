...
/**
 * @file Database/Markers/PlatformLanguagesTable.php
 *
 * @description
 * Migration marker for the `platform_languages` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(PlatformLanguagesTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `platform_languages`. The resolver uses this as a tiebreaker among
...
final class PlatformLanguagesTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_08_01_000010_create_platform_languages_table.php';
