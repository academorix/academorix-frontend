...
1. **`OtelServiceProvider`** (traces + metrics)
   ...
   - W3C Baggage propagator (for `tenant_id` / `causer_id` cross-service)
...
- resumes the trace so the consumer span links to the producer.
  ...
  inside async jobs carry the originating tenant_id + causer_id.

**Baggage:** Keys like `tenant_id`, `causer_id` propagate via `#[AsBaggage]` so
downstream services / async jobs see them without manual passing.
...
dropped (bounded queue) and `stackra.otel.dropped_spans` increments. Log sinks
buffer with the same policy: bounded queue, oldest-dropped, drop counter in
metrics.
