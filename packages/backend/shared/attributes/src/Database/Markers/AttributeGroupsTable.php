...
/**
 * @file Database/Markers/AttributeGroupsTable.php
 *
 * @description
 * Migration marker for the `attribute_groups` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(AttributeGroupsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `attribute_groups`. The resolver uses this as a tiebreaker among
...
final class AttributeGroupsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120001_create_attribute_groups_table.php';
