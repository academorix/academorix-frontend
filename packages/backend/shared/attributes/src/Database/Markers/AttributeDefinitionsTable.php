...
/**
 * @file Database/Markers/AttributeDefinitionsTable.php
 *
 * @description
 * Migration marker for the `attribute_definitions` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(AttributeDefinitionsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `attribute_definitions`. The resolver uses this as a tiebreaker among
...
final class AttributeDefinitionsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120000_create_attribute_definitions_table.php';
