...
/**
 * @file Database/Markers/SearchIndexesTable.php
 *
 * @description
 * Migration marker for the `search_indexes` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(SearchIndexesTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `search_indexes`. The resolver uses this as a tiebreaker among
...
final class SearchIndexesTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_000100_create_search_indexes_table.php';
