...
/**
 * @file Database/Markers/SearchSyncJobsTable.php
 *
 * @description
 * Migration marker for the `search_sync_jobs` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(SearchSyncJobsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `search_sync_jobs`. The resolver uses this as a tiebreaker among
...
final class SearchSyncJobsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_000101_create_search_sync_jobs_table.php';
