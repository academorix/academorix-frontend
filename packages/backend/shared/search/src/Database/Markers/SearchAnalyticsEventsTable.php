...
/**
 * @file Database/Markers/SearchAnalyticsEventsTable.php
 *
 * @description
 * Migration marker for the `search_analytics_events` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(SearchAnalyticsEventsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `search_analytics_events`. The resolver uses this as a tiebreaker among
...
final class SearchAnalyticsEventsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_000104_create_search_analytics_events_table.php';
