...
/**
 * @file Database/Markers/SearchSynonymsTable.php
 *
 * @description
 * Migration marker for the `search_synonyms` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(SearchSynonymsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `search_synonyms`. The resolver uses this as a tiebreaker among
...
final class SearchSynonymsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_000102_create_search_synonyms_table.php';
