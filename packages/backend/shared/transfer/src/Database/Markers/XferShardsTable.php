...
/**
 * @file Database/Markers/XferShardsTable.php
 *
 * @description
 * Migration marker for the `xfer_shards` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(XferShardsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `xfer_shards`. The resolver uses this as a tiebreaker among
...
final class XferShardsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_000111_create_xfer_shards_table.php';
