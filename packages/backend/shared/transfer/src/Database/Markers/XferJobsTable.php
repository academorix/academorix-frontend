...
/**
 * @file Database/Markers/XferJobsTable.php
 *
 * @description
 * Migration marker for the `xfer_jobs` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(XferJobsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `xfer_jobs`. The resolver uses this as a tiebreaker among
...
final class XferJobsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_000110_create_xfer_jobs_table.php';
