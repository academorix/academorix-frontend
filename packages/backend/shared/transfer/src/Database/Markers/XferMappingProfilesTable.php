...
/**
 * @file Database/Markers/XferMappingProfilesTable.php
 *
 * @description
 * Migration marker for the `xfer_mapping_profiles` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(XferMappingProfilesTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `xfer_mapping_profiles`. The resolver uses this as a tiebreaker among
...
namespace Stackra\Transfer\Database\Markers;

use Stackra\Database\Attributes\DependsOn;

final class XferMappingProfilesTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_000113_create_xfer_mapping_profiles_table.php';
