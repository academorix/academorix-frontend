...
/**
 * @file Database/Markers/XferArtifactsTable.php
 *
 * @description
 * Migration marker for the `xfer_artifacts` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(XferArtifactsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `xfer_artifacts`. The resolver uses this as a tiebreaker among
...
final class XferArtifactsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_000112_create_xfer_artifacts_table.php';
