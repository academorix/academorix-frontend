...
/**
 * @file Database/Markers/ApiVersionsTable.php
 *
 * @description
 * Migration marker for the `api_versions` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(ApiVersionsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `api_versions`. The resolver uses this as a tiebreaker among
...
final class ApiVersionsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_000080_create_api_versions_table.php';
