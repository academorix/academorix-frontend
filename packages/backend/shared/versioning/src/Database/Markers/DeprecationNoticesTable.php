...
/**
 * @file Database/Markers/DeprecationNoticesTable.php
 *
 * @description
 * Migration marker for the `deprecation_notices` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(DeprecationNoticesTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `deprecation_notices`. The resolver uses this as a tiebreaker among
...
namespace Stackra\Versioning\Database\Markers;

use Stackra\Database\Attributes\DependsOn;

final class DeprecationNoticesTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_000081_create_deprecation_notices_table.php';
