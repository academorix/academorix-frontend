...
Channel-driver module for the mail (email) notification transport. Registers the
`mail` channel with the parent notifications module's channel registry.
...
- 1 attribute: `#[AsNotificationChannel]` — attribute-driven registration of the
  channel driver into the parent's `NotificationChannelRegistry`. Mirrors the
  sibling notifications-in-app copy verbatim.
- 4 tenant-facing actions: list / show / add / remove suppression.
- 1 central-plane action: `ReceiveMailWebhook` (public provider webhook receiver
  — signature-verified, dispatches ingest job).
- 5 commands: `notifications:mail:test-send`,
  `notifications:mail:reconcile-suppressions`,
  `notifications:mail:suppression-add`, `notifications:mail:suppression-remove`,
...
- 2 listeners: `HandleMailDispatch` (filters `channel = mail`) and
  `IngestMailProviderWebhookListener` (filters `webhook::InboundWebhookReceived`
  by namespace).
