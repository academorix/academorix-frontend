...
- Blade rendering of pre-rendered React Email HTML (React Email compiled at CI
  build time via `packages/notifications-emails-renderer`).
- Feature flags per provider (kill switches for `mailgun`, `ses`, `sendgrid`,
  `postmark`, `resend`).

### Compatibility

- Depends on `notifications` (core), `foundation`, `tenants`.
