...
/**
 * @file Database/Markers/MailSuppressionsTable.php
 *
 * @description
 * Migration marker for the `mail_suppressions` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(MailSuppressionsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `mail_suppressions`. The resolver uses this as a tiebreaker among
...
namespace Stackra\Notifications\Mail\Database\Markers;

use Stackra\Database\Attributes\DependsOn;

final class MailSuppressionsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_000120_create_mail_suppressions_table.php';
