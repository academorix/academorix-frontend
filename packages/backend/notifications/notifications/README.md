...
- **Attributes**: `#[Notifiable]` (mark a category-owner module),
  `#[HasPreferences]` (mark a model that carries per-user preferences), plus
  `#[NotificationCategory]` and `#[NotificationTemplate]` for attribute-driven
  registry hydration.
- **Permissions**: `NotificationsPermission` — dual-guard (Sanctum + platform
  admin).
...
- **Rules (4)**: `template_key_format`, `supported_channel`, `valid_locale`,
  `recipient_shape`.
...
`NotificationDispatched` after commit. Every consumer downstream is event-driven
— no cross-module DB reads.

## Microservice readiness

Every design choice above keeps the door open for a future extraction to a Node
microservice: event-carried state (every payload includes the recipient
