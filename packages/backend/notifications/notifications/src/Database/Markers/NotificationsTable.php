...
/**
 * @file Database/Markers/NotificationsTable.php
 *
 * @description
 * Migration marker for the `notifications` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(NotificationsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `notifications`. The resolver uses this as a tiebreaker among
...
final class NotificationsTable
{
    ...
    public const string MIGRATION = '2026_07_15_000120_create_notifications_table.php';
