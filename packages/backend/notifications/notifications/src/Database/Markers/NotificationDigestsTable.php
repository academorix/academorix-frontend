...
/**
 * @file Database/Markers/NotificationDigestsTable.php
 *
 * @description
 * Migration marker for the `notification_digests` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(NotificationDigestsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `notification_digests`. The resolver uses this as a tiebreaker among
...
final class NotificationDigestsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_000125_create_notification_digests_table.php';
