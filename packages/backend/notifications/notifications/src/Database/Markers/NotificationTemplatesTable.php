...
/**
 * @file Database/Markers/NotificationTemplatesTable.php
 *
 * @description
 * Migration marker for the `notification_templates` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(NotificationTemplatesTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `notification_templates`. The resolver uses this as a tiebreaker among
...
final class NotificationTemplatesTable
{
    ...
    public const string MIGRATION = '2026_07_15_000122_create_notification_templates_table.php';
