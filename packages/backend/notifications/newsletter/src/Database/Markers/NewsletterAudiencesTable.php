...
/**
 * @file Database/Markers/NewsletterAudiencesTable.php
 *
 * @description
 * Migration marker for the `newsletter_audiences` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(NewsletterAudiencesTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `newsletter_audiences`. The resolver uses this as a tiebreaker among
...
final class NewsletterAudiencesTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_000203_create_newsletter_audiences_table.php';
