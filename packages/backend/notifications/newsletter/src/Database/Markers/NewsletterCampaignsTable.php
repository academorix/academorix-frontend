...
/**
 * @file Database/Markers/NewsletterCampaignsTable.php
 *
 * @description
 * Migration marker for the `newsletter_campaigns` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(NewsletterCampaignsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `newsletter_campaigns`. The resolver uses this as a tiebreaker among
...
final class NewsletterCampaignsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_000204_create_newsletter_campaigns_table.php';
