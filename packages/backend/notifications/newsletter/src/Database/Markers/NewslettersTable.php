...
/**
 * @file Database/Markers/NewslettersTable.php
 *
 * @description
 * Migration marker for the `newsletters` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(NewslettersTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `newsletters`. The resolver uses this as a tiebreaker among
...
final class NewslettersTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_000200_create_newsletters_table.php';
