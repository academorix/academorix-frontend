...
/**
 * @file Database/Markers/NewsletterSubscriptionsTable.php
 *
 * @description
 * Migration marker for the `newsletter_subscriptions` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(NewsletterSubscriptionsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `newsletter_subscriptions`. The resolver uses this as a tiebreaker among
...
final class NewsletterSubscriptionsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_000202_create_newsletter_subscriptions_table.php';
