...
/**
 * @file Database/Markers/SmsOptOutsTable.php
 *
 * @description
 * Migration marker for the `sms_opt_outs` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(SmsOptOutsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `sms_opt_outs`. The resolver uses this as a tiebreaker among
...
namespace Stackra\Notifications\Sms\Database\Markers;

use Stackra\Database\Attributes\DependsOn;

final class SmsOptOutsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_000120_create_sms_opt_outs_table.php';
