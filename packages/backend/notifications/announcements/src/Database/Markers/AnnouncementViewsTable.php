...
/**
 * @file Database/Markers/AnnouncementViewsTable.php
 *
 * @description
 * Migration marker for the `announcement_views` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(AnnouncementViewsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `announcement_views`. The resolver uses this as a tiebreaker among
...
namespace Stackra\Announcements\Database\Markers;

use Stackra\Database\Attributes\DependsOn;

final class AnnouncementViewsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120000_create_announcement_views_table.php';
