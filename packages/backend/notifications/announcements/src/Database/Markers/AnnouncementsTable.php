...
/**
 * @file Database/Markers/AnnouncementsTable.php
 *
 * @description
 * Migration marker for the `announcements` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(AnnouncementsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `announcements`. The resolver uses this as a tiebreaker among
...
final class AnnouncementsTable
{
    ...
    public const string MIGRATION = '2026_07_15_120001_create_announcements_table.php';
