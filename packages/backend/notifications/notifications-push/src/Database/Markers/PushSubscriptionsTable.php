...
/**
 * @file Database/Markers/PushSubscriptionsTable.php
 *
 * @description
 * Migration marker for the `push_subscriptions` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(PushSubscriptionsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `push_subscriptions`. The resolver uses this as a tiebreaker among
...
namespace Stackra\Notifications\Push\Database\Markers;

use Stackra\Database\Attributes\DependsOn;

final class PushSubscriptionsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_000110_create_push_subscriptions_table.php';
