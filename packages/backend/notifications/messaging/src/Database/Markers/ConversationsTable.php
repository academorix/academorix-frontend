...
/**
 * @file Database/Markers/ConversationsTable.php
 *
 * @description
 * Migration marker for the `conversations` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(ConversationsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `conversations`. The resolver uses this as a tiebreaker among
...
final class ConversationsTable
{
    ...
    public const string MIGRATION = '2026_07_15_120001_create_conversations_table.php';
