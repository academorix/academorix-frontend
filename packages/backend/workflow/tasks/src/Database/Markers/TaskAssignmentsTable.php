...
/**
 * @file Database/Markers/TaskAssignmentsTable.php
 *
 * @description
 * Migration marker for the `task_assignments` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(TaskAssignmentsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `task_assignments`. The resolver uses this as a tiebreaker among
...
final class TaskAssignmentsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120000_create_task_assignments_table.php';
