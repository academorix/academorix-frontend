...
/**
 * @file Database/Markers/ApprovableActionsTable.php
 *
 * @description
 * Migration marker for the `approvable_actions` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(ApprovableActionsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `approvable_actions`. The resolver uses this as a tiebreaker among
...
final class ApprovableActionsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120000_create_approvable_actions_table.php';
