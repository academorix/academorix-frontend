...
/**
 * @file Database/Markers/ApprovalRemindersTable.php
 *
 * @description
 * Migration marker for the `approval_reminders` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(ApprovalRemindersTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `approval_reminders`. The resolver uses this as a tiebreaker among
...
final class ApprovalRemindersTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120003_create_approval_reminders_table.php';
