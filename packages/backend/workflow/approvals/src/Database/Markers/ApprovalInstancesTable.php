...
/**
 * @file Database/Markers/ApprovalInstancesTable.php
 *
 * @description
 * Migration marker for the `approval_instances` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(ApprovalInstancesTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `approval_instances`. The resolver uses this as a tiebreaker among
...
final class ApprovalInstancesTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120002_create_approval_instances_table.php';
