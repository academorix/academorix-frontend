...
/**
 * @file Database/Markers/ApprovalTemplateApproversTable.php
 *
 * @description
 * Migration marker for the `approval_template_approvers` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(ApprovalTemplateApproversTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `approval_template_approvers`. The resolver uses this as a tiebreaker among
...
namespace Stackra\Approvals\Database\Markers;

use Stackra\Database\Attributes\DependsOn;

final class ApprovalTemplateApproversTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120005_create_approval_template_approvers_table.php';
