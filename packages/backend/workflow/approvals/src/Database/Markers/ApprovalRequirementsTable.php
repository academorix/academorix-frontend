...
/**
 * @file Database/Markers/ApprovalRequirementsTable.php
 *
 * @description
 * Migration marker for the `approval_requirements` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(ApprovalRequirementsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `approval_requirements`. The resolver uses this as a tiebreaker among
...
final class ApprovalRequirementsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120004_create_approval_requirements_table.php';
