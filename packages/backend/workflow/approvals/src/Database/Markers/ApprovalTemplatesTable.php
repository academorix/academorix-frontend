...
/**
 * @file Database/Markers/ApprovalTemplatesTable.php
 *
 * @description
 * Migration marker for the `approval_templates` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(ApprovalTemplatesTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `approval_templates`. The resolver uses this as a tiebreaker among
...
final class ApprovalTemplatesTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120006_create_approval_templates_table.php';
