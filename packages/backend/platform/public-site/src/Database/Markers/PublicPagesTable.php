...
/**
 * @file Database/Markers/PublicPagesTable.php
 *
 * @description
 * Migration marker for the `public_pages` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(PublicPagesTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `public_pages`. The resolver uses this as a tiebreaker among
...
namespace Stackra\PublicSite\Database\Markers;

use Stackra\Database\Attributes\DependsOn;

final class PublicPagesTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120001_create_public_pages_table.php';
