...
/**
 * @file Database/Markers/ContentBlocksTable.php
 *
 * @description
 * Migration marker for the `content_blocks` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(ContentBlocksTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `content_blocks`. The resolver uses this as a tiebreaker among
...
final class ContentBlocksTable
{
    ...
    public const string MIGRATION = '2026_07_15_120000_create_content_blocks_table.php';
