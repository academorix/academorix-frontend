...
/**
 * @file Database/Markers/AiDraftsTable.php
 *
 * @description
 * Migration marker for the `ai_drafts` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(AiDraftsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `ai_drafts`. The resolver uses this as a tiebreaker among
...
final class AiDraftsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120001_create_ai_drafts_table.php';
