...
/**
 * @file Database/Markers/AiToolCallsTable.php
 *
 * @description
 * Migration marker for the `ai_tool_calls` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(AiToolCallsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `ai_tool_calls`. The resolver uses this as a tiebreaker among
...
final class AiToolCallsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120004_create_ai_tool_calls_table.php';
