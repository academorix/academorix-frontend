...
/**
 * @file Database/Markers/AiEmbeddingsTable.php
 *
 * @description
 * Migration marker for the `ai_embeddings` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(AiEmbeddingsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `ai_embeddings`. The resolver uses this as a tiebreaker among
...
final class AiEmbeddingsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120002_create_ai_embeddings_table.php';
