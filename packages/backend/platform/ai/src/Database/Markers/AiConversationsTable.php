...
/**
 * @file Database/Markers/AiConversationsTable.php
 *
 * @description
 * Migration marker for the `ai_conversations` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(AiConversationsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `ai_conversations`. The resolver uses this as a tiebreaker among
...
namespace Stackra\Ai\Database\Markers;

use Stackra\Database\Attributes\DependsOn;

final class AiConversationsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120000_create_ai_conversations_table.php';
