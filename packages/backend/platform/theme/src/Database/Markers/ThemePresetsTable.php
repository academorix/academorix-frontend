...
/**
 * @file Database/Markers/ThemePresetsTable.php
 *
 * @description
 * Migration marker for the `theme_presets` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(ThemePresetsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `theme_presets`. The resolver uses this as a tiebreaker among
...
final class ThemePresetsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120000_create_theme_presets_table.php';
