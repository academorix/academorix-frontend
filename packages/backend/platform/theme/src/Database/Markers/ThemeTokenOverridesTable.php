...
/**
 * @file Database/Markers/ThemeTokenOverridesTable.php
 *
 * @description
 * Migration marker for the `theme_token_overrides` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(ThemeTokenOverridesTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `theme_token_overrides`. The resolver uses this as a tiebreaker among
...
namespace Stackra\Theme\Database\Markers;

use Stackra\Database\Attributes\DependsOn;

final class ThemeTokenOverridesTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120000_create_theme_token_overrides_table.php';
