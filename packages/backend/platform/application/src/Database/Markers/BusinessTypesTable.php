...
/**
 * @file Database/Markers/BusinessTypesTable.php
 *
 * @description
 * Migration marker for the `business_types` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(BusinessTypesTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `business_types`. The resolver uses this as a tiebreaker among
...
final class BusinessTypesTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_000002_create_business_types_table.php';
