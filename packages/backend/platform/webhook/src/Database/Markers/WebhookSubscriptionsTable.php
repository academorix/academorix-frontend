...
/**
 * @file Database/Markers/WebhookSubscriptionsTable.php
 *
 * @description
 * Migration marker for the `webhook_subscriptions` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(WebhookSubscriptionsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `webhook_subscriptions`. The resolver uses this as a tiebreaker among
...
namespace Stackra\Webhook\Database\Markers;

use Stackra\Database\Attributes\DependsOn;

final class WebhookSubscriptionsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_000070_create_webhook_subscriptions_table.php';
