...
/**
 * @file Database/Markers/WebhookDeliveriesTable.php
 *
 * @description
 * Migration marker for the `webhook_deliveries` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(WebhookDeliveriesTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `webhook_deliveries`. The resolver uses this as a tiebreaker among
...
final class WebhookDeliveriesTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_000071_create_webhook_deliveries_table.php';
