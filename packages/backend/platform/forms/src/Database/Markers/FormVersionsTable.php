...
/**
 * @file Database/Markers/FormVersionsTable.php
 *
 * @description
 * Migration marker for the `form_versions` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(FormVersionsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `form_versions`. The resolver uses this as a tiebreaker among
...
namespace Stackra\Forms\Database\Markers;

use Stackra\Database\Attributes\DependsOn;

final class FormVersionsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120001_create_form_versions_table.php';
