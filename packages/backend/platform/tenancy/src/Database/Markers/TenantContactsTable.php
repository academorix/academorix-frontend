...
/**
 * @file Database/Markers/TenantContactsTable.php
 *
 * @description
 * Migration marker for the `tenant_contacts` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(TenantContactsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `tenant_contacts`. The resolver uses this as a tiebreaker among
...
namespace Stackra\Tenancy\Database\Markers;

use Stackra\Database\Attributes\DependsOn;

final class TenantContactsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_000011_create_tenant_contacts_table.php';
