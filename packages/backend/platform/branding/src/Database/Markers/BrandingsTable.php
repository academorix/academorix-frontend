...
/**
 * @file Database/Markers/BrandingsTable.php
 *
 * @description
 * Migration marker for the `brandings` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(BrandingsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `brandings`. The resolver uses this as a tiebreaker among
...
namespace Stackra\Branding\Database\Markers;

use Stackra\Database\Attributes\DependsOn;

final class BrandingsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_000030_create_brandings_table.php';
