...
/**
 * @file Database/Markers/BroadcastSubscriptionsTable.php
 *
 * @description
 * Migration marker for the `broadcast_subscriptions` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(BroadcastSubscriptionsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `broadcast_subscriptions`. The resolver uses this as a tiebreaker among
...
namespace Stackra\Realtime\Database\Markers;

use Stackra\Database\Attributes\DependsOn;

final class BroadcastSubscriptionsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120001_create_broadcast_subscriptions_table.php';
