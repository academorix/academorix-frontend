...
/**
 * @file Database/Markers/BroadcastChannelsTable.php
 *
 * @description
 * Migration marker for the `broadcast_channels` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(BroadcastChannelsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `broadcast_channels`. The resolver uses this as a tiebreaker among
...
final class BroadcastChannelsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120000_create_broadcast_channels_table.php';
