...
/**
 * @file Database/Markers/ReceptionVisitsTable.php
 *
 * @description
 * Migration marker for the `reception_visits` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(ReceptionVisitsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `reception_visits`. The resolver uses this as a tiebreaker among
...
namespace Stackra\Reception\Database\Markers;

use Stackra\Database\Attributes\DependsOn;

final class ReceptionVisitsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120000_create_reception_visits_table.php';
