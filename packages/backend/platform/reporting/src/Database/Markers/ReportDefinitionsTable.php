...
/**
 * @file Database/Markers/ReportDefinitionsTable.php
 *
 * @description
 * Migration marker for the `report_definitions` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(ReportDefinitionsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `report_definitions`. The resolver uses this as a tiebreaker among
...
namespace Stackra\Reporting\Database\Markers;

use Stackra\Database\Attributes\DependsOn;

final class ReportDefinitionsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120001_create_report_definitions_table.php';
