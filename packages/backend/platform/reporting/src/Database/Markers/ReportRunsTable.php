...
/**
 * @file Database/Markers/ReportRunsTable.php
 *
 * @description
 * Migration marker for the `report_runs` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(ReportRunsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `report_runs`. The resolver uses this as a tiebreaker among
...
final class ReportRunsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120002_create_report_runs_table.php';
