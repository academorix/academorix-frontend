...
/**
 * @file Database/Markers/IntegrationProvidersTable.php
 *
 * @description
 * Migration marker for the `integration_providers` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(IntegrationProvidersTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `integration_providers`. The resolver uses this as a tiebreaker among
...
final class IntegrationProvidersTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120003_create_integration_providers_table.php';
