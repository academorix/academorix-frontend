...
/**
 * @file Database/Markers/AppWebhookSubscriptionsTable.php
 *
 * @description
 * Migration marker for the `app_webhook_subscriptions` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(AppWebhookSubscriptionsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `app_webhook_subscriptions`. The resolver uses this as a tiebreaker among
...
namespace Stackra\Integrations\Database\Markers;

use Stackra\Database\Attributes\DependsOn;

final class AppWebhookSubscriptionsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120001_create_app_webhook_subscriptions_table.php';
