...
/**
 * @file Database/Markers/AppInstallationsTable.php
 *
 * @description
 * Migration marker for the `app_installations` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(AppInstallationsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `app_installations`. The resolver uses this as a tiebreaker among
...
final class AppInstallationsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120000_create_app_installations_table.php';
