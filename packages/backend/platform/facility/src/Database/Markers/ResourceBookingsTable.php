...
/**
 * @file Database/Markers/ResourceBookingsTable.php
 *
 * @description
 * Migration marker for the `resource_bookings` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(ResourceBookingsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `resource_bookings`. The resolver uses this as a tiebreaker among
...
namespace Stackra\Facility\Database\Markers;

use Stackra\Database\Attributes\DependsOn;

final class ResourceBookingsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120003_create_resource_bookings_table.php';
