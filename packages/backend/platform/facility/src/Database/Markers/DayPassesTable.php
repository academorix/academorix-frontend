...
/**
 * @file Database/Markers/DayPassesTable.php
 *
 * @description
 * Migration marker for the `day_passes` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(DayPassesTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `day_passes`. The resolver uses this as a tiebreaker among
...
final class DayPassesTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120000_create_day_passes_table.php';
