...
/**
 * @file Database/Markers/CheckinLogsTable.php
 *
 * @description
 * Migration marker for the `checkin_logs` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(CheckinLogsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `checkin_logs`. The resolver uses this as a tiebreaker among
...
namespace Stackra\Credentials\Database\Markers;

use Stackra\Database\Attributes\DependsOn;

final class CheckinLogsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120000_create_checkin_logs_table.php';
