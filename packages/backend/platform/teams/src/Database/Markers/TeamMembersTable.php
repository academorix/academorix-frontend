...
/**
 * @file Database/Markers/TeamMembersTable.php
 *
 * @description
 * Migration marker for the `team_members` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(TeamMembersTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `team_members`. The resolver uses this as a tiebreaker among
...
final class TeamMembersTable
{
    ...
    public const string MIGRATION = '2026_07_15_120001_create_team_members_table.php';
