...
/**
 * @file Database/Markers/FileVariantsTable.php
 *
 * @description
 * Migration marker for the `file_variants` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(FileVariantsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `file_variants`. The resolver uses this as a tiebreaker among
...
final class FileVariantsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_000061_create_file_variants_table.php';
