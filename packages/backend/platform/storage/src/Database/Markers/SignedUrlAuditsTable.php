...
/**
 * @file Database/Markers/SignedUrlAuditsTable.php
 *
 * @description
 * Migration marker for the `signed_url_audits` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(SignedUrlAuditsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `signed_url_audits`. The resolver uses this as a tiebreaker among
...
namespace Stackra\Storage\Database\Markers;

use Stackra\Database\Attributes\DependsOn;

final class SignedUrlAuditsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_000062_create_signed_url_audits_table.php';
