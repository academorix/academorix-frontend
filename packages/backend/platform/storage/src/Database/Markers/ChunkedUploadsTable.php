...
/**
 * @file Database/Markers/ChunkedUploadsTable.php
 *
 * @description
 * Migration marker for the `chunked_uploads` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(ChunkedUploadsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `chunked_uploads`. The resolver uses this as a tiebreaker among
...
final class ChunkedUploadsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_000063_create_chunked_uploads_table.php';
