...
/**
 * @file Database/Markers/PersonGuardianLinksTable.php
 *
 * @description
 * Migration marker for the `person_guardian_links` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(PersonGuardianLinksTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `person_guardian_links`. The resolver uses this as a tiebreaker among
...
final class PersonGuardianLinksTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120000_create_person_guardian_links_table.php';
