...
/**
 * @file Database/Markers/PersonIdentitiesTable.php
 *
 * @description
 * Migration marker for the `person_identities` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(PersonIdentitiesTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `person_identities`. The resolver uses this as a tiebreaker among
...
final class PersonIdentitiesTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120001_create_person_identities_table.php';
