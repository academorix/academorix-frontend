...
`POST /api/v1/people/lookup` takes `{ stackra_id, date_of_birth }`. The response
contains ONLY `{ found: true, request_url? }` — no PII, no name, no nationality
is revealed before consent. If a match is found, the caller may file a
`TenantLinkRequest` which fires a consent notification to the linked guardians.
