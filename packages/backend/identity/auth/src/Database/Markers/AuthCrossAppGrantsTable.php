...
/**
 * @file Database/Markers/AuthCrossAppGrantsTable.php
 *
 * @description
 * Migration marker for the `auth_cross_app_grants` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(AuthCrossAppGrantsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `auth_cross_app_grants`. The resolver uses this as a tiebreaker among
...
final class AuthCrossAppGrantsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120000_create_auth_cross_app_grants_table.php';
