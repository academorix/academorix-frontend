...
/**
 * @file Database/Markers/AuthJwtSigningKeysTable.php
 *
 * @description
 * Migration marker for the `auth_jwt_signing_keys` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(AuthJwtSigningKeysTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `auth_jwt_signing_keys`. The resolver uses this as a tiebreaker among
...
final class AuthJwtSigningKeysTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120003_create_auth_jwt_signing_keys_table.php';
