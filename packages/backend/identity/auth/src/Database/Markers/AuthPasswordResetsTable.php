...
/**
 * @file Database/Markers/AuthPasswordResetsTable.php
 *
 * @description
 * Migration marker for the `auth_password_resets` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(AuthPasswordResetsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `auth_password_resets`. The resolver uses this as a tiebreaker among
...
final class AuthPasswordResetsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120005_create_auth_password_resets_table.php';
