...
/**
 * @file Database/Markers/AuthJwtDenyListsTable.php
 *
 * @description
 * Migration marker for the `auth_jwt_deny_lists` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(AuthJwtDenyListsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `auth_jwt_deny_lists`. The resolver uses this as a tiebreaker among
...
final class AuthJwtDenyListsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120002_create_auth_jwt_deny_lists_table.php';
