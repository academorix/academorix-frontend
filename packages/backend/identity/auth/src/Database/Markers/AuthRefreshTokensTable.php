...
/**
 * @file Database/Markers/AuthRefreshTokensTable.php
 *
 * @description
 * Migration marker for the `auth_refresh_tokens` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(AuthRefreshTokensTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `auth_refresh_tokens`. The resolver uses this as a tiebreaker among
...
final class AuthRefreshTokensTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120006_create_auth_refresh_tokens_table.php';
