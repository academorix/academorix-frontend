...
/**
 * @file Database/Markers/PlatformProfilesTable.php
 *
 * @description
 * Migration marker for the `platform_profiles` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(PlatformProfilesTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `platform_profiles`. The resolver uses this as a tiebreaker among
...
final class PlatformProfilesTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120000_create_platform_profiles_table.php';
