...
/**
 * @file Database/Markers/ServiceAccountsTable.php
 *
 * @description
 * Migration marker for the `service_accounts` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(ServiceAccountsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `service_accounts`. The resolver uses this as a tiebreaker among
...
namespace Stackra\ServiceAccounts\Database\Markers;

use Stackra\Database\Attributes\DependsOn;

final class ServiceAccountsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120000_create_service_accounts_table.php';
