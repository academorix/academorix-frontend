...
/**
 * @file Database/Markers/IdentitiesTable.php
 *
 * @description
 * Migration marker for the `identities` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(IdentitiesTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `identities`. The resolver uses this as a tiebreaker among
...
namespace Stackra\Identity\Database\Markers;

use Stackra\Database\Attributes\DependsOn;

final class IdentitiesTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_120000_create_identities_table.php';
