...
/**
 * @file Database/Markers/WebauthnCredentialsTable.php
 *
 * @description
 * Migration marker for the `webauthn_credentials` table. Discovered at boot by
 * {@see \Stackra\Database\Migrations\MigrationDagResolver} —
 * every downstream marker that references THIS one via
 * `#[DependsOn(WebauthnCredentialsTable::class)]` must run AFTER the migration
 * named in the MIGRATION constant.
 *
 * ## What this marker declares
 *
 *   * `MIGRATION` — the timestamped migration file that CREATES
 *     `webauthn_credentials`. The resolver uses this as a tiebreaker among
...
namespace Stackra\Mfa\Database\Markers;

use Stackra\Database\Attributes\DependsOn;

final class WebauthnCredentialsTable
{
    /** Migration file that produces this table. */
    public const string MIGRATION = '2026_07_15_115959_create_webauthn_credentials_table.php';
