...
- `use HasProfile;` — should be a compositional trait on this package that adds
  the `Profile` (1:1 satellite of the User) relation + Profile accessors.
...
- `UserProvisioner` — atomic create of User + Profile row for a (identity_id,
  application_id) tuple. Called from `Auth` on first login.
- `UserFinder` — case-insensitive email lookup within an Application scope. Used
  by invitation acceptance.
