...
Error monitoring for the Stackra framework — a fan-out manager over
pluggable providers (Sentry, console, or your own), with auto-registration
and React bindings.
...
`captureMessage` / `addBreadcrumb` / `setUser` fans out to the `stack`
(default: all configured instances). `provider(name)` gives named access.
Providers init eagerly in `onApplicationBootstrap`. A throwing provider is
isolated.
